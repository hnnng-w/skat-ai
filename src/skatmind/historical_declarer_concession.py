...
from skatmind.historical_game_end import HistoricalDeclarerConcession
...
def adjudicate_historical_declarer_concession(
    record: Any,
    replay: HistoricalReplayState,
) -> dict[str, Any]:
    """Adjudicates one exact historical concession through shared domain behavior."""
    event = record.game_end
    if not isinstance(event, HistoricalDeclarerConcession):
        raise ValueError("Historical declarer-concession adjudication requires its event.")

    reconstructed_count = len(replay.remaining_hand_for(record.declarer_player_id))
    if event.declarer_hand_cards_remaining != reconstructed_count:
        raise ValueError(
            "Historical game_end.declarer_hand_cards_remaining contradicts exact "
            f"play-prefix reconstruction: expected {reconstructed_count}, got "
            f"{event.declarer_hand_cards_remaining}."
        )
    ...
    shared_concession = DeclarerConcession(
    ...
    result = {
        ...
        "winner": "defenders",
        "schneider_status": "not_applicable",
