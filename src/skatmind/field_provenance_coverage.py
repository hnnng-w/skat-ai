...
def _canonicalize_path_tuple(value: object, *, path: str) -> tuple[str, ...]:
    if isinstance(value, (str, bytes)) or not isinstance(value, (list, tuple)):
        raise SkatMindValidationError(f"{path} must be an immutable-compatible array.", path=path)
    copied = tuple(value)
    for item in copied:
        if not isinstance(item, str):
            raise SkatMindValidationError(f"{path} must contain JSON Pointer strings.", path=path)
