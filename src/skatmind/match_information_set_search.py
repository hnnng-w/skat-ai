...
def build_match_information_set_search_request_fields_v1(
    ...
    if serialized is None:
        raise SkatMindInvariantError("Match Information-set Search settings were not serialized.")
...
def _require_mapping(
    ...
    if not isinstance(value, Mapping):
        raise SkatMindInvariantError(f"Match Information-set Search Result requires {field_name}.")
    return value
...
def _require_sequence(
    ...
    if isinstance(value, (str, bytes)) or not isinstance(value, Sequence):
        raise SkatMindInvariantError(f"Match Information-set Search {field_name} must be an array.")
    return value
...
def _require_mapping_value(
    ...
    if not isinstance(value, Mapping):
        raise SkatMindInvariantError(
            f"Match Information-set Search {field_name} must be an object."
        )
    return value
...
def _validate_safe_search_result(
    *,
    ...
    if game_type != request_document.get("game_type") or not isinstance(game_type, str):
        raise SkatMindInvariantError(
            "Match Information-set Search changed the requested game type."
        )
    try:
    ...
    if legal_cards != expected_legal_cards:
        raise SkatMindInvariantError("Match Information-set Search Result changed legal Cards.")
    ...
    if coverage == "none":
        ...
        raise SkatMindInvariantError("Match Information-set Search coverage is unsupported.")
    ...
    if controlled_policy_count != consumed_budget.controlled_policy_decisions:
        raise SkatMindInvariantError(
            "Match Information-set Search controlled-Policy counts differ."
        )
    ...
    if status == "complete":
        ...
        raise SkatMindInvariantError("Match Information-set Search status is unsupported.")
...
def _validate_comparison_semantics(
    *,
    ...
    if any(
        ...
        raise SkatMindInvariantError("Match Information-set Search comparison ranks differ.")
...
def reconcile_match_information_set_search_result_v1(
    *,
    ...
    if list(fixed_policies) != expected_policies:
        raise SkatMindInvariantError(
            "Match Information-set Search changed effective fixed Policies."
        )
    ...
    if summary != expected_summary:
        raise SkatMindInvariantError("Match Information-set Search method summary is inconsistent.")
    if recommendation.get("card") != recommended_card:
        raise SkatMindInvariantError("Match Information-set Search recommendation Cards differ.")
    if sum(
...
def build_match_information_set_search_report_view_v1(
    ...
    if not isinstance(search, Mapping) or not isinstance(comparison, Mapping):
        raise SkatMindInvariantError("Match Information-set Search diagnostics are incomplete.")
    consumed = _require_mapping(search, "consumed_budget")
    return {
        "status": search.get("status"),
        "stop_reason": search.get("stop_reason"),
