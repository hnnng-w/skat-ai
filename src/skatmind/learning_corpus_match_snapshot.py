...
def _derive_learning_corpus_references_v1(
    ...
    *,
    ...
    for slot in workspace.slots:
        ...
        if game is None:
            continue
        game_content_fingerprint = build_learning_corpus_game_content_fingerprint_v1(
            game,
            _legacy_identity=legacy_identity,
        )
...
def _strictly_verify_source_document_v1(
    ...
    except (SkatMindValidationError, AttributeError, KeyError, TypeError, ValueError) as error:
        path = error.path if isinstance(error, SkatMindValidationError) else ""
...
def build_learning_corpus_match_snapshot_v1(
    ...
    legacy_identity = document.document_kind == LEGACY_MATCH_WORKSPACE_DOCUMENT_KIND
    try:
...
def validate_learning_corpus_match_snapshot_v1(
    ...
    for document_kind in (
        ...
        except (SkatMindInvariantError, AttributeError, KeyError, TypeError, ValueError):
            continue
        if rebuilt == snapshot:
            matches.append(rebuilt)
    if len(matches) != 1:
        raise SkatMindInvariantError(
            "Learning Corpus Match Snapshot does not equal one supported exact derivation.",
            path="",
        )
