...
from skatmind.session_history import _reconstruct_session_prefix_v1
...
_SESSION_STATE_FINGERPRINT_DOMAIN = b"skatmind\0session_state_v1\0"
_SESSION_PERSISTENCE_FINGERPRINT_DOMAIN = b"skatmind\0session_persistence_v1\0"
_LEGACY_SESSION_STATE_FINGERPRINT_DOMAIN = b"skat-ai\0session_state_v1\0"
_LEGACY_SESSION_PERSISTENCE_FINGERPRINT_DOMAIN = b"skat-ai\0session_persistence_v1\0"
...
def _fingerprint_profile_for_document_kind(
    ...
    if document_kind == SESSION_PERSISTENCE_DOCUMENT_KIND:
        return (
            _SESSION_STATE_FINGERPRINT_DOMAIN,
            _SESSION_PERSISTENCE_FINGERPRINT_DOMAIN,
            SESSION_PERSISTENCE_DOCUMENT_KIND,
        )
    if document_kind == LEGACY_SESSION_PERSISTENCE_DOCUMENT_KIND:
        return (
            _LEGACY_SESSION_STATE_FINGERPRINT_DOMAIN,
            _LEGACY_SESSION_PERSISTENCE_FINGERPRINT_DOMAIN,
            LEGACY_SESSION_PERSISTENCE_DOCUMENT_KIND,
        )
    _raise_validation("document_kind is unsupported.", path="/document_kind")


def _build_replayed_state_fingerprint_v1(
    state: SessionStateV1,
    *,
    fingerprint_domain: bytes = _SESSION_STATE_FINGERPRINT_DOMAIN,
) -> str:
...
def _build_session_persistence_content_fingerprint_v1(
    *,
    ...
    decision_checkpoints: tuple[SessionDecisionCheckpointV1, ...],
    document_kind: str = SESSION_PERSISTENCE_DOCUMENT_KIND,
    fingerprint_domain: bytes = _SESSION_PERSISTENCE_FINGERPRINT_DOMAIN,
) -> str:
    return _sha256_domain_fingerprint(
        fingerprint_domain,
...
def _validate_session_persistence_document_fingerprints_v1(
    ...
    state_domain, persistence_domain, document_kind = (
        _fingerprint_profile_for_document_kind(document.document_kind)
    )
    try:
        replay_session_state_v1(document.state)
        state_fingerprint = _build_replayed_state_fingerprint_v1(
            document.state,
            fingerprint_domain=state_domain,
        )
...
def _build_request(value: object, *, path: str) -> RequestDocumentV1:
    ...
    try:
        ...
        raise SkatMindValidationError(str(error), path=f"{path}/document") from error
