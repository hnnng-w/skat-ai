...
from skatmind.field_provenance import FieldProvenanceEntry, FieldProvenanceLedger
...
if TYPE_CHECKING:
    from skatmind.application.contracts import ApplicationInvocation
...
class V1InformationProvenanceSourceBinding:
    ...
    def __post_init__(self) -> None:
        from skatmind.application.contracts import _freeze_json_object
...
def _request_entry(
    ...
    from skatmind.field_provenance import parse_json_pointer
...
def build_v1_information_provenance_sources(
    ...
    if not isinstance(request_document, dict):
        raise SkatMindInvariantError("Verified Root Request did not thaw to an object.")
...
def consumed_v1_request_document(
    ...
    if request_attachment is None:
        raise SkatMindInvariantError("Provenance sources have no consumed Request.")
...
def validate_v1_information_provenance_sources(
    ...
    if sources.workflow is not invocation.request.workflow:
        raise SkatMindInvariantError("Consumed provenance source workflow changed.")
    by_name = {item.name: item for item in sources.attachments}
    if len(by_name) != len(sources.attachments):
        raise SkatMindInvariantError("Consumed provenance source attachment is duplicated.")
    ...
    if request is None or options is None:
        raise SkatMindInvariantError("Required consumed provenance source is missing.")
    if not exact_v1_json_equal(
        request.document_to_dict(),
        invocation.request.to_dict()["document"],
    ):
        raise SkatMindInvariantError("Verified Root Request provenance source changed.")
    request_document = invocation.request.document
    request_context = _request_context(invocation.request.workflow, request_document)
    ...
    if (
        ...
        raise SkatMindInvariantError("Verified Root Request provenance ledger changed.")
    ...
    if not expects_local_consumed and retained_consumed is not None:
        raise SkatMindInvariantError("Unexpected local consumed Request is retained.")
    if expects_local_consumed:
        if retained_consumed is None:
            raise SkatMindInvariantError("Local consumed Request is missing.")
        ...
        if (
            ...
            raise SkatMindInvariantError("Local consumed Request provenance changed.")
    options_document = build_v1_application_options_document(invocation)
    if not exact_v1_json_equal(options.document_to_dict(), options_document):
        raise SkatMindInvariantError("Application-option provenance source changed.")
    ...
    if (
        ...
        raise SkatMindInvariantError("Application-option provenance ledger changed.")
    ...
    if external is None and retained_external is not None:
        raise SkatMindInvariantError("Unexpected external provenance source is retained.")
    if external is not None and (
        ...
        raise SkatMindInvariantError("Injected external provenance source changed.")
    if retained_external is not None:
        ...
        if (
            ...
            raise SkatMindInvariantError("Injected external provenance ledger changed.")
