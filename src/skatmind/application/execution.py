...
from skatmind.api.v1.contracts import RequestDocumentV1, ResultDocumentV1, WorkflowV1
from skatmind.application.contracts import (
...
from skatmind.application.provenance import ApplicationProvenanceBundle
from skatmind.application.simple_workflows import (
...
from skatmind.errors import SkatMindInvariantError, SkatMindWorkflowError
...
def build_application_invocation(
    ...
    *,
    ...
    if not isinstance(root_document, Mapping):
        raise SkatMindWorkflowError("Root document must be an object.")
...
def _validate_positive_sample_count(value: int | None, name: str) -> None:
    if value is None:
        return
    if value <= 0:
        raise SkatMindWorkflowError(f"{name} must be a positive integer.")
...
def _validate_position_options(
    ...
    if options.multi_step_count is not None and options.multi_step_count <= 0:
        raise SkatMindWorkflowError("multi_step_count must be a positive integer.")
    ...
    if options.compare_policies and options.multi_step_count is None:
        raise SkatMindWorkflowError("compare_policies requires multi_step_count.")
...
def validate_application_invocation(invocation: ApplicationInvocation) -> None:
    ...
    if not isinstance(invocation, ApplicationInvocation):
        raise SkatMindWorkflowError("invocation must be an ApplicationInvocation.")
...
def _position_handler(
    ...
    *,
    ...
    if options is None:
        raise SkatMindInvariantError("Position handler received no Position options.")
    ...
    if not exact_v1_json_equal(
        ...
        raise SkatMindInvariantError("Application consumed external input changed.")
    provenance = (
...
def _statistics_handler(
    ...
    from skatmind.opponent_workflow_provenance import (
...
def _list_handler(
    ...
    from skatmind.historical_list_provenance import HistoricalListProvenanceCollector
...
def _execute_application_invocation(
    ...
    *,
    ...
    if not exact_v1_json_equal(request_data, expected_consumed_request):
        raise SkatMindInvariantError("Application consumed Request input changed.")
    ...
    if result.workflow is not workflow:
        raise SkatMindInvariantError("Application result workflow identity changed.")
