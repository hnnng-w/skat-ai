...
from skat_ai.match_workspace_progress import _build_validated_match_workspace_progress_v1
...
_WORKSPACE_FINGERPRINT_DOMAIN = b"skat-ai\0match_workspace_v1\0"
_PERSISTENCE_FINGERPRINT_DOMAIN = b"skat-ai\0match_workspace_persistence_v1\0"
...
def _build_tournament_format(
    ...
    *,
    ...
    try:
        ...
        raise SkatAIValidationError(str(error), path=f"{path}/format_id") from error
...
def _build_snapshot(
    ...
    *,
    ...
    try:
        ...
        raise SkatAIValidationError(str(error), path=f"{path}/statistics_record") from error
    snapshot = _construct(
        MatchPlayerStatisticsSnapshotV1,
        path=path,
...
def _build_validated_workspace_fingerprint_v1(workspace: MatchWorkspaceV1) -> str:
...
def resume_match_workspace_document_v1(
    ...
    if data["document_kind"] != MATCH_WORKSPACE_DOCUMENT_KIND:
        _raise_validation(
            f"document_kind must equal {MATCH_WORKSPACE_DOCUMENT_KIND!r}.",
            path="/document_kind",
        )
