...
def _recorded_play_count(summary: Mapping[str, object]) -> int:
    ...
    return sum(
        ...
        and isinstance(trick.get("plays", ()), (list, tuple))
    )
...
def _review_entry(
    ...
    *,
    ...
    if top == "historical_opponent_profile_application_summary":
        references = (
            ...
            if external_reference is not None
                ...
                result_source_reference("algorithm", "historical_profile_application"),
            )
        )
    ...
    return result_provenance_entry(
        ...
        source_references=(
            result_source_reference("aggregate", "historical_review_summary"),
        ),
    ...
    *,
    ...
    if implied_by is not None:
        return result_provenance_entry(
            ...
            dependency_paths=(
                f"/historical_game_summary/record/declaration/{implied_by}",
            ),
        )
    return result_provenance_entry(
        path,
...
def _record_entry(
    ...
    *,
    ...
    private_deal = (
        "initial_hand" in record_tokens
        or record_tokens[:1] in {("skat",), ("discarded_cards",)}
    )
    if private_deal:
        return result_provenance_entry(
            path,
    ...
    if record_tokens[:1] == ("tricks",):
        if len(record_tokens) >= 4 and record_tokens[1].isdecimal() and (
            record_tokens[2] == "plays"
        ):
...
def _incomplete_trick_entry(
    ...
    *,
    ...
    return result_provenance_entry(
        path,
        origin=(
            "rule_derived" if path.endswith("/next_player_id") else "historical_replay"
        ),
        visibility="public",
        available_from="after_actual_play",
        derivation=(
            "deterministic_rule" if path.endswith("/next_player_id") else "reconstruction"
        ),
        source_references=(
            result_source_reference("algorithm", "historical_legal_replay_v1"),
...
def _terminal_entry(
    ...
    *,
    ...
    if record_end_reason == "open_card_throw" and (
        "theoretical_schwarz_assessment" in path
        or "jack_ownership_evidence" in path
    ):
        references.append(
            result_source_reference(
                "algorithm",
...
def _event_summary_entry(
    ...
    *,
    ...
    event_rows = (
        events_summary.get("events", ())
        if isinstance(events_summary, Mapping)
        else ()
    )
    event_kind = None
    if isinstance(event_rows, (list, tuple)) and event_rows and isinstance(
        event_rows[0], Mapping
    ):
...
def _base_summary_entry(
    ...
    *,
    ...
    if branch in {
        ...
        return _point_entry(path, leaf_paths=leaf_paths, game_id=game_id)
    ...
    if branch in {"winner", "schneider_status", "schwarz_status"}:
        return result_provenance_entry(
            ...
            derivation="deterministic_rule",
            source_references=(
                result_source_reference("rule_contract", "historical_game_result_v1"),
            ),
            dependency_paths=leaf_paths_below(
                leaf_paths,
