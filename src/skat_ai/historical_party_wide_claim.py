...
from skat_ai.party_wide_claim_adjudication import (
...
def _build_exact_proof_summary(proof: Any) -> dict[str, Any]:
    if proof.assignment is None:
        raise SkatAIInvariantError("Valid party-wide Claim proof lacks its assignment.")
    return {
...
def _build_adjudication_summary(adjudication: Any) -> dict[str, Any]:
    ...
    if facts is None:
        raise SkatAIInvariantError("Valid party-wide Claim lacks adjudication Facts.")
    return {
...
def adjudicate_historical_party_wide_claim(
    ...
    if event.schema_version != HISTORICAL_PARTY_WIDE_CLAIM_SCHEMA_VERSION:
        raise SkatAIInvariantError("Historical party-wide Claim schema version drifted.")
    ...
    if not all(
        ...
        raise SkatAIInvariantError("Adjudicated party-wide Claim summaries are incomplete.")
