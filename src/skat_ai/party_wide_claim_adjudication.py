...
from skat_ai.rules import get_card_points, get_legal_cards, get_trick_winner
from skat_ai.settlement_normative_matrix import PARTY_WIDE_ALL_REMAINING_TRICKS_CLAIM_V1
...
def adjudicate_party_wide_claim_proof_v1(
    ...
    if evidence is None or assignment is None:
        raise SkatAIInvariantError("Valid party-wide Claim proof lacks adjudication inputs.")
    ...
    if observed_declarer_points + observed_defender_points + evidence.unresolved_card_points != 120:
        raise SkatAIInvariantError("Claim point accounting does not total 120 points.")
    ...
    if final_declarer_points + final_defender_points != 120:
        raise SkatAIInvariantError("Adjudicated Claim points do not total 120.")
    ...
    if len(final_winner_parties) != 10 or final_declarer_tricks + final_defender_tricks != 10:
        raise SkatAIInvariantError("Adjudicated Claim Trick ownership does not total ten.")
    ...
    if winner not in {"declarer", "defenders"}:
        raise SkatAIInvariantError("Completed Claim assignment did not decide the Game.")
