...
def _aggregate_utility_key(
    ...
    if any(item is None for item in margins):
        raise SkatAIInvariantError("Suit and Grand utility aggregation requires margins.")
...
class _ExecutionController:
    ...
    def _depth(self, state: InformationSetSearchWorldStateV1) -> int:
        ...
        if depth < 0:
            raise SkatAIInvariantError("A World State predates the prepared Search root.")
        return depth
...
def _advance_fixed_players(
    ...
    while True:
        ...
        if observation is None:
            raise SkatAIInvariantError("A non-terminal World lacks an actor Observation.")
...
def _solve_bundle(
    ...
    if all(item.terminal_utility is not None for item in evaluations):
        ...
        if len(solution.terminal_utilities) != len(frontier):
            raise SkatAIInvariantError("A terminal World bundle lost selected-draw weight.")
        controller.bundle_memo[states] = solution
        return solution
    ...
    if information_set is None or information_set.actor_player != "me":
        raise SkatAIInvariantError("The controlled frontier does not belong to me.")
    ...
    if best_card is None or best_solution is None:
        raise SkatAIInvariantError("A controlled Information Set produced no legal outcome.")
    ...
    *,
    ...
    for card, solution in solutions_by_card:
        ...
        if len(utilities) != selected_world_count:
            raise SkatAIInvariantError("A root Candidate lost selected-draw weight.")
        success_count = sum(int(item.local_contract_success) for item in utilities)
        game_score_sum = sum(item.local_side_game_score for item in utilities)
...
def _consumed_budget(
    *,
    ...
    if selection is None:
        raise SkatAIInvariantError("Available execution lost its World selection.")
...
def execute_information_set_search_v1(
    ...
    if selection is None or root_information_set is None or local_side is None:
        raise SkatAIInvariantError("Available execution lacks required root facts.")
    ...
    try:
        ...
        if any(item.observation != root_information_set for item in root_evaluations):
            raise SkatAIInvariantError("Prepared root Information Sets changed at execution.")
        ...
        if best_card is None:
            raise SkatAIInvariantError("Root execution produced no Candidate.")
