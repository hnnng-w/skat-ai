...
from skat_ai.match_workspace_persistence_codec import resume_match_workspace_document_v1
...
_CATALOG_FINGERPRINT_DOMAIN = b"skat-ai\0learning_corpus_catalog_v1\0"
_PERSISTENCE_FINGERPRINT_DOMAIN = b"skat-ai\0learning_corpus_persistence_v1\0"
...
def _build_validated_catalog_fingerprint_v1(catalog: LearningCorpusCatalogV1) -> str:
    return _sha256_domain_fingerprint(_CATALOG_FINGERPRINT_DOMAIN, catalog.to_dict())
...
def _persistence_content_fingerprint_material(
    *,
    ...
    return {
        ...
        "document_kind": LEARNING_CORPUS_CATALOG_DOCUMENT_KIND,
...
def _validate_learning_corpus_catalog_persistence_document_fingerprints_v1(
    ...
    catalog_fingerprint = _build_validated_catalog_fingerprint_v1(document.catalog)
...
def resume_learning_corpus_catalog_document_v1(
    ...
    if data["document_kind"] != LEARNING_CORPUS_CATALOG_DOCUMENT_KIND:
        _raise_validation(
            f"document_kind must equal {LEARNING_CORPUS_CATALOG_DOCUMENT_KIND!r}.",
            path="/document_kind",
        )
...
def resume_learning_corpus_match_snapshot_object_v1(
    ...
    if rebuilt.to_dict() != dict(data):
        _raise_validation(
            "Persisted Match Snapshot does not equal its canonical Workspace derivation.",
            path="",
        )
    return rebuilt
