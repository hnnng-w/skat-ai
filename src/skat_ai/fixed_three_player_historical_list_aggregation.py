...
def _validate_count_and_formula_invariants(value: Any, field_prefix: str) -> None:
    ...
    if value.defender_game_count != (
        value.defender_games_won + value.defender_games_lost
    ):
...
def _validate_progression_delta(
    ...
    if snapshot.aggregation_version != (
    ...
    previous_by_player_id = (
        ...
        if previous is None
            ...
            total.player_id: total for total in previous.cumulative_player_totals
        }
    )
    ...
    for total in snapshot.cumulative_player_totals:
        ...
        for field_name in _NUMERIC_FIELDS:
            ...
            if getattr(total, field_name) - previous_value != getattr(
...
def _build_progression(
    ...
    for fact in facts:
        ...
        settlement_sum = sum(
            item.settlement_score or 0
            for item in prefix_facts
            if item.entry_kind == "played_game"
        )
        _reconcile_totals(
            player_totals,
...
def _validate_historical_list_source(
    ...
    if (
        ...
        or historical_list.schema_version
        != FIXED_THREE_PLAYER_HISTORICAL_LIST_SCHEMA_VERSION
    ):
    ...
    if not isinstance(historical_list.players, tuple) or len(
    ...
    if not isinstance(historical_list.entries, tuple) or len(
    ...
    if _resolve_canonical_player_labels(
        historical_list.players,
        historical_list.entries,
    ) != historical_list.players:
...
def _validate_final_aggregation(
    ...
    *,
    ...
    if aggregation.ranking_status not in (
        FIXED_THREE_PLAYER_HISTORICAL_LIST_RANKING_STATUSES
    ):
        raise ValueError("Unsupported final ranking status.")
    unresolved_tie = bool(aggregation.tied_player_ids) and (
        aggregation.applied_lot_order is None
    )
    expected_status = "lot_required" if unresolved_tie else "final"
    expected_lot_required_ids = (
        aggregation.tied_player_ids if unresolved_tie else ()
    )
    ...
    expected_standings = build_fixed_three_player_historical_list_standings(
        ...
        lot_order=(
            None
            if aggregation.applied_lot_order is None
            else list(aggregation.applied_lot_order)
        ),
    )
    ...
    *,
    ...
    result = FixedThreePlayerHistoricalListAggregation(
        ...
        lot_required_player_ids=(
            standings_result.tied_player_ids if unresolved_tie else ()
        ),
        applied_lot_order=standings_result.applied_lot_order,
        final_standings=standings_result.standings,
    )
    settlement_score_sum = sum(
        fact.settlement_score or 0
        for fact in facts
        if fact.entry_kind == "played_game"
    )
    _validate_final_aggregation(result, settlement_score_sum=settlement_score_sum)
    return result
