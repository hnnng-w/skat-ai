...
conflict Reload. Materialization, Public Match API, Match Schema/data workflow,
Player Statistics application, and analysis remain absent. `v1.0.0` remains
...
| Requirement | Source | Rule section | Current status | Current implementation | Required input or information | Known limitation | Required validation or tests | Target milestone | Required before v1.0 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Match Capture identity and metadata | skat-ai product | Not applicable | `partially_supported` | Issue #160 defines internal immutable version-1 Match Capture, Source Metadata, Media Timecode, Tournament Format, Participant, and Player Statistics Snapshot values. Issue #163 retains the definition inside one exact Workspace, Issue #164 consumes it through internal Capture Application services, and Issue #165 creates or corrects the permitted fields through private browser forms. | Caller-supplied Match identity/title/platform, optional external ID and played time, one canonical format, one descriptive source, exactly three participants, one perspective Player, and optional previously retained statistics snapshots. | Materialization, Public Match API, Match Schema/data workflow, Player Statistics editing/application, analysis, YouTube integration, and EuroSkat integration remain absent. Ranking, qualification, prize, fee, and bonus rules are not implemented. | Retain source relationships and no-network behavior, exact registry identity/order, snapshot identity/time reconciliation and preservation, fixed-place and perspective relationships, browser creation/correction, uniqueness, unchanged public exports, and unchanged Package/Schema/scenario counts. | v0.15.0 usable local capture complete; downstream use open | Yes |
...
| Persistent EuroSkat Match Workspace | skat-ai product | Not applicable | `partially_supported` | Issue #163 defines exact 36-Slot Workspaces, rotation, revisions, Progress, fingerprints, strict Resume, and optimistic atomic Save. Issue #165 adds one locked explicit-file context, absent creation, strict startup Resume, exact-revision operation plus content-fingerprint CAS autosave, unchanged/conflict no-write behavior, and explicit Reload. | One exact Match definition, optional observed Games/passed deals, exact revisions, one explicit existing-parent file path, and retained fingerprint. | Structural completion does not imply evidence completion. No materialization, Statistics application, Public Match API/Schema, distributed lock, retry/merge, remote deployment, cloud, encryption, or backup exists. | Retain all Workspace/persistence bytes and rules plus one operation/at-most-one Save, no context replacement on conflict, explicit-only Reload, private path/fingerprint omission, and unchanged public/count boundaries. | v0.15.0 usable local persistence complete; downstream use open | Yes |
| Rapid post-game Match Capture Application services | skat-ai product | Not applicable | `partially_supported` | Issue #164 defines transport-free Card entries, Views, Results, setup, automatic Play append, truncation/annotation cleanup, annotation editing, and pass/clear wrappers. Issue #165 maps browser operations directly to these services and preserves exact versus bounded Card wording. | One loaded Workspace, selected position, exact revision, and operation-specific observed evidence. | Services themselves still perform no I/O. Public Match API/Schema/data workflow, Statistics application, materialization, analysis, tactical interpretation, and AI commentary remain absent. Bounded candidates do not assert ownership or legality. | Retain all Issue #164 identities/behavior, direct delegation, conflict precedence, atomic batches, automatic actors/indexes, no hidden completion, import direction, and unchanged persistence/public/count boundaries. | v0.15.0 usable local transport complete; broader product open | Yes |
| Local Match Capture browser and CLI transport | skat-ai product | Not applicable | `supported` | Issue #165 adds internal Match Capture Web, Protocol, and CLI version `1`; exact 17-operation order and six policy strings; leading `capture` dispatch; required one-file CLI; loopback Standard Library HTTP server; token/cookie/same-origin/Host/request-limit/security-header controls; deterministic path-free state and server HTML; packaged local assets; no-JavaScript forms plus keyboard/focus enhancement; and Wheel/sdist clean-install smoke. | One explicit Workspace path with existing parent, browser startup token/cookie, exact local origin, selected position, exact revision, and operation form or JSON body. | Private local transport only. No remote bind, account, access-control or encryption claim, external request, Public Match API/Schema/Root workflow, analysis, materialization, Statistics editing, YouTube/EuroSkat integration, database, cloud, or backup. | Retain exact versions/policies/routes/options, 36 positions, creation/Resume/setup/Card/correction/annotation/pass/clear flows, exact/bounded labels, autosave counts/conflicts/Reload, security/privacy, packaged resources, one Console Script, clean installs, and unchanged 0.14.0/63/85/public boundaries. | v0.15.0 bounded local interface complete | Yes |
