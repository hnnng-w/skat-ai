...
Statistics history, and strict time-safe selection. Deletion and garbage
collection, Player Catalog persistence, persisted aliases/assertions, merge/split,
all-revision views, Commentary/Response export, target separation, Dataset
...
| Private Learning Corpus identity, Catalog, persistence, Workspace import, and derived Player history | skat-ai product | Not applicable | `partially_supported` | Issues #171 and #172 define immutable Match Snapshots/references, authoritative Catalog/Current selections, fixed-root strict persistence, orphan reporting, pure Catalog changes, no-clobber objects, atomic Save, and Workspace import. Issue #173 derives a non-persisted Current-Snapshot-only Player Catalog with exact stable identity, label history, participant/online-source aliases and conflicts, complete exact Statistics observations, shared source-Match temporal status, and latest-unambiguous or explicit-observation as-of selection. | One exact strict Store Result; persistence operations additionally require the explicit root/source/expected revision and fingerprint options. Statistics selection requires exact stable Player ID, target time, mode, and an explicit observation ID where applicable. | No deletion, garbage collection, recovery UI, Player Catalog persistence, persisted aliases/assertions, merge/split, all-revision view, report import, Commentary export, Dataset version `2`, browser/CLI/API/Schema, or model training exists. Orphans are reported but never repaired or included automatically. | Retain exact versions/tuples/policies/domains, strict JSON and canonical bytes, Current-only and orphan exclusion, exact identity/label/alias behavior, record fingerprints/provenance, strict RFC 3339 eligibility and ambiguity, no merge/Profile/I/O/analysis behavior, persistence compatibility, and unchanged 0.15.0/63/6/85 baselines. | v0.16.0 private identity, persistence, and Player-history foundations complete; later Corpus layers open | No |
