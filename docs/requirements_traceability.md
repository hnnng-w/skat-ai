...
| Complete-game retrospective analysis | skat-ai product | Not applicable | `partially_supported` | A validated normal-play historical game can produce 30 ordered information-safe pre-play states with stable actual-card labels, but those states are not yet evaluated by the recommendation workflow. | Complete historical record and a validated replay result. | No per-decision recommendations, quality ratings, or complete-game coaching report exist. | Evaluate reconstructed snapshots through the approved recommendation workflow without weakening leakage protections. | v1.0 | Yes |
| Historical-game representation | skat-ai product | Not applicable | `partially_supported` | A separate version-1 workflow preserves stable game/player IDs, fixed seats, all initial cards, final declaration, pickup/discards or Hand state, and all 30 plays for normal completion; it derives winners, points, result, overbid, settlement, and optional decision-time snapshots and round-trips the canonical record. | Complete `historical_game_input` record ending with `normal_completion`. | Full auction events, claims/concessions and other end reasons, disputes, recommendation-based replay analysis, and training wrappers are absent. | Retain schema/runtime, identity, deal, ownership, legality, snapshot leakage, round-trip, CLI, example, output, and settlement tests; add later approved end reasons separately. | v1.0 | Yes |
