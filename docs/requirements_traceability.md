...
Position propagation and adversarial enforcement. Retrospective, non-Position,
...
| Field-level information provenance | skat-ai product | Not applicable | `partially_supported` | Internal contract version `1` defines canonical RFC 6901 paths, immutable ledgers, coverage, dependencies, Information Use Context, redaction, serialization, and Confidence separation. Application provenance version `1` and live-analysis provenance version `1` add complete flat and simulated decision ledgers plus an all-leaf partial-legacy Position Result ledger for live Immediate, Search, inference, Multi-Step, Policy Comparison, continuation, and external-profile paths. | A live Position Application invocation, its exact local/public decision states and serialized Result, and matching internal sidecars. | Retrospective Position, Historical, Coaching, Dataset, list, Opponent, and public provenance integration remain absent. Settlement, rating, list, terminal, and retrospective Position Result branches remain explicit legacy exemptions. | Retain core contract tests plus complete decision coverage, all Search statuses and auto routing, Ouvert and continuations, profile dependencies, context denial before execution, private-root invariance, redaction, deterministic ordering, unchanged call counts, no public exports, unchanged schemas/CLI, and 70 generated outputs. | v0.13.0 live Position propagation implemented; broader propagation remains open | Yes |
| Live information boundaries | skat-ai product | Not applicable | `partially_supported` | Existing broad enforcement remains, and live Position Application execution now validates complete pre-selection decision ledgers and attaches a complete-coverage partial-legacy Result ledger. Declared Ouvert and both continuation branches authorize only their exact current public hands. Accepted 4.4.5 proof hands, other hands, hidden skat, future cards, private worlds, and derived seeds remain unavailable. | Explicit mode, local perspective, verifiable public history, valid declared-Ouvert or continuation public hands, effective seed-free settings, and the exact Application Result. | General position input still trusts supplied context; retrospective and public provenance integration remain open, and out-of-scope Result branches remain legacy exemptions. | Retain decision-time context enforcement, all-leaf accounting, rejection/redaction, Ouvert and continuation exceptions, hidden proof evidence, post-game and private-world leakage, determinism, call-count, and public-boundary tests. | v1.0 | Yes |
