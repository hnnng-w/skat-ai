...
status. Public Match API and Schema/data workflow, global Player
...
persisted explicit Current selection. Deletion and garbage collection, a Player
Catalog, multi-Match Player Snapshot history, Commentary/Response export, target
separation, Dataset version `2`, annotations, cross-game summaries, evaluation
...
| Private Learning Corpus identity, Catalog, persistence, and Workspace import | skat-ai product | Not applicable | `partially_supported` | Issue #171 defines immutable content-addressed Match Snapshots, closed References, lightweight Catalog entries, explicit current selections, and duplicate/revision classification. Issue #172 adds one fixed explicit Corpus root, deterministic Catalog/content fingerprints, strict Catalog and Snapshot reconstruction, strict Store Resume with valid sorted orphan reporting, pure Catalog import and selection changes, immutable no-clobber object publication, optimistic atomic Catalog Save, strict source-preserving Workspace-file import, and persisted selection updates. | One explicit Corpus root with existing parent, caller-supplied Corpus ID for initialization, strict source Workspace file, expected Catalog revision/content fingerprint, explicit selection mode, and explicit same-revision resolution. | No deletion, garbage collection, recovery UI, Player Catalog, cross-revision lineage, report import, Commentary export, Dataset version `2`, browser/CLI/API/Schema, or model training exists. Orphans are reported but never repaired or deleted automatically. | Retain exact versions/tuples/layout/policies/domains, strict JSON and canonical bytes, referenced-object reconciliation, orphan behavior, conflict precedence, pure import semantics, no-clobber and atomic failure preservation, object-before-Catalog conflict reuse, source immutability, no-I/O/no-analysis boundaries, and unchanged 0.15.0/63/6/85 compatibility. | v0.16.0 private identity and persistence foundations complete; later Corpus layers open | No |
