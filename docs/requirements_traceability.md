...
| Bidding and declarations | ISkO | 3.3.1-3.3.11; 3.5.1-3.5.6 | `partially_supported` | `GameDeclaration` represents the final contract and bid value; runtime validation checks types and Null exclusions. | Final game type, declaration modifiers, optional matadors, and optional bid value. | No auction sequence, bid/hold/pass model, declarer derivation, legal bid-value validation, passed-in game, or complete declaration dependency checks. | Add final-declaration dependency tests for v1.0; a full auction requires a separate product decision and tests. | v1.0 plus product decision | Final declaration: Yes; full auction: Decision required |
| Suit and Grand game values | ISkO | 2.4.1; 2.5.1-2.5.8 | `partially_supported` | Base values and multiplier calculation are implemented in `game_declaration.py` and `game_value.py`. | Valid final declaration and known matador count. | Invalid official modifier combinations can be represented; matador bounds and all declaration dependencies are not fully enforced. | Parameterize every base value, legal level, invalid dependency, and boundary count. | v1.0 | Yes |
...
| Matadors | ISkO | 2.3.1-2.3.4; 2.5.2-2.5.3 | `partially_supported` | Explicit matadors and conservative inference from known declarer cards and concrete completed-trick ownership are tested. | Explicit count or deterministic declarer ownership including the skat where known. | No exhaustive reconstruction from a complete game; ambiguous ownership remains incomplete; legal Suit/Grand maxima are not fully validated. | Cover with/without sequences, Suit maxima, Grand maxima, Hand skat effects, complete histories, and ambiguous cases. | v1.0 | Yes |
| Hand games | ISkO | 2.1.1-2.1.2; 2.6.1-2.6.4; 3.5.1 | `partially_supported` | `hand_game` is represented and contributes one game-value level. | Final declaration and cards needed for valuation. | The engine cannot prove the skat was not inspected and does not enforce every Hand-only announcement dependency. | Reject illegal pickup/Hand metadata and illegal announcement combinations; test skat-dependent final valuation. | v1.0 | Yes |
| Schneider and Schwarz | ISkO | 2.5.4-2.5.8 | `partially_supported` | Card-point Schneider and ten-trick ownership-based Schwarz affect settlement; announced levels are modeled. | Complete points; ten reliable trick owners for Schwarz; declaration flags. | Claims do not establish Schwarz; invalid announcement dependencies remain possible; full rule-violation outcomes are absent. | Test both parties, zero-point tricks, announcements, higher achieved levels, incomplete histories, and invalid declarations. | v1.0 | Yes |
| Ouvert declarations | ISkO | 2.5.8-2.5.9; 2.6.5; 3.5.1 | `partially_supported` | `ouvert` is represented; Null values and Suit/Grand multiplier contribution are calculated. | Final declaration and completed result. | Suit/Grand ouvert is not forced to Hand/Schwarz-announced semantics; exposure timing and card layout are not represented. | Reject invalid combinations and test Suit/Grand all-trick and Null no-trick outcomes. | v1.0 | Yes |
...
| Impossible Null declarations | ISkO | 3.6.2 | `not_supported` | Null overbid is detected, but required value and settlement remain incomplete. | Last bid, declarer matadors, final declaration, and an approved Suit/Grand selection interpretation. | ISkO 3.6.2 requires a lost Suit or Grand but does not state a deterministic software selection rule. | Obtain authoritative guidance or approve and document a product convention, then add settlement tests. | v1.0 | Yes |
...
| Final settlement | ISkO | 2.5.1-2.5.11; 3.6.1-3.6.4 | `partially_supported` | `final_settlement.py` covers wins, doubled losses, achieved/announced levels, Null variants, and supported Suit/Grand overbids. | Complete result, valid declaration, game value, bid, and reliable trick ownership where required. | Source and output explicitly call the logic simplified; impossible Null, declaration dependencies, claims, concessions, and rule violations leave compliance gaps. | Build a normative contract/level/outcome table and settlement regression suite for every supported variant. | v1.0 | Yes |
...
* ISkO 3.6.2 does not identify which Suit or Grand contract software should use
  when an impossible Null declaration becomes a loss. `skat-ai` must obtain an
  authoritative interpretation or document an approved product convention
  before implementing this settlement.
