...
63-Schema Package parity, and clean-install validation. Session-triggered
analysis, actual-card Checkpoint attachment, public file Save/Load, CLI,
examples/generated output, automatic Checkpoint collection, end-to-end capture,
and UI remain open. Online-platform adapters, browser extensions, and website
...
strict Command parsing, all ten one-call in-memory operations, typed Results,
optional complete redacted Session Provenance, and strict standalone Session
Schema. Public persistence build/resume is in-memory only. The active tree has
seven unchanged Root workflows, 63 authoritative and packaged Schemas, 77
generated outputs, and Package version `0.13.0`; the published `v0.13.0` baseline
remains 62 Schemas.
In the matrix below, prior internal persistence/resume and stale-write-detection
limitations are implemented by Issue #155. Issue #156 supersedes wording that
calls the entire Session API, Provenance, or Schema absent; public file transport
and end-to-end Session capture remain open. See
[Session persistence and resume](session_persistence_and_resume.md).

| Requirement | Source | Rule section | Current status | Current implementation | Required input or information | Known limitation | Required validation or tests | Target milestone | Required before v1.0 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Public in-memory Session API | skat-ai product | Not applicable | `supported` | Issue #156 defines stable `skat_ai.api.v1.session` version `1`, exact immutable type identity, all ten one-call operations, strict Command parsing, typed Results, optional complete returned-value provenance, and standalone Schema validation. | Existing typed Session values or strict JSON-object Command/persistence mappings. | No Session Root workflow, automatic analysis, public file Save/Load, CLI Session command, automatic/actual-card Checkpoint attachment, example/generated output, or UI. | Retain exact exports, operation/value pairing, normal statuses, one-call delegation, default provenance omission, complete redacted coverage, strict Schema, 63-Schema parity, and clean-install execution. | v0.14.0 bounded public interface complete | Yes |
| Stable public Python and installed CLI contract | skat-ai product | Not applicable | `supported` | API contract version `1` defines the stable namespaces, seven Root workflows, immutable documents and options, public Results and artifacts, errors, compatibility metadata, and flattened envelope. Issue #147 adds immutable public field-provenance types, default-false `include_provenance`, typed `ExecutionResultV1.field_provenance`, Root `field_provenance`, and unchanged envelope keys. Installed, module, and Legacy forms share `--include-provenance`, concise summary, quiet behavior, and Root JSON parity. Package, API, Application, CLI, Schema, Provenance, and Domain versions remain independent. | A Root JSON file and optional CLI transport paths, or one already loaded JSON-compatible Root object or immutable Request with matching options and optional paired Opponent Statistics. | Broader Domain-error migration, license selection, Package-index publication, public or executable interactive Sessions, and end-to-end field enforcement remain absent. | Retain exact exports, immutable contracts, all seven workflows, default omission and opt-in parity, concise/quiet CLI output, errors and normal unavailable states, Legacy seams, 62-schema package parity, and 77 generated outputs without changing the historical 70-scenario `v0.12.0` baseline. | v0.13.0 bounded release scope complete | Yes |
...
| Interactive Session capture | skat-ai product | Not applicable | `partially_supported` | Issue #150 defines internal Session and Command version `1`, exactly three stable seated Players, Live and Retrospective Modes, phases, ten typed Commands, an authoritative accepted Log, linear revision and promotion relationships, Diagnostics, Position/Historical readiness, valid-incomplete State, Transition Result semantics, and deterministic immutable serialization. Issue #151 adds transition and projection version `1`, canonical revision-zero creation, full accepted-Log replay, atomic application/rejection, revision-conflict precedence, monotonic phases, incremental Deal-through-end rule and information validation, exact/bounded ownership, trick/event/end derivation, promotion, readiness calculation, and forged-State detection. Issue #152 adds Session Request Export version `1`, immutable available/unavailable Results, one-replay Historical readiness gating, exact Retrospective mapping through the existing Historical builder and canonical round trip, and immutable existing Request construction without execution. Issue #153 adds the narrow declared-Ouvert public-hand Command, immutable Position Export Options, one-replay information-safe Position mapping through the existing builder, and replay-verified frozen pre-Play Decision Checkpoints. Issue #154 adds immutable strict-prefix Undo, one-command correction, deterministic first-rejection suffix replay, valid partial corrected States, and exact Checkpoint lineage. Issue #155 adds private deterministic Session persistence/resume with strict reconstruction, accepted-Log replay, fingerprints, recomputed lineage, optimistic conflict detection, and canonical atomic files. | Caller-supplied Session identity, three stable Players and seats, Capture Mode, local Player for Live, and typed Commands with expected revisions. Historical export requires a fully ready Retrospective State with exact Deal, Declaration, legal Play prefix, Game ID, and supported ending facts. Position export requires one ready local decision, an exact local hand, explicit analysis options, and any required opponent-Ouvert public hand. Private persistence additionally accepts an authoritative replay-valid State, caller-supplied frozen Checkpoints, a file path, and the expected existing content fingerprint. | No Session-triggered analysis or end-to-end capture workflow, actual-card Checkpoint attachment, Public API, Session Provenance, Schema, CLI Session Assistant, example, generated output, automatic Checkpoint collection, or UI exists. | Retain exact constants/policies, constructors, recursive immutability, deterministic projection/State/export/Checkpoint/persistence serialization, conflict-before-semantics, atomic unchanged-State rejection, strict-prefix history editing, Live information restrictions, Retrospective ownership and Bedienpflicht, all declarations, public-hand sources, continuations/endings and chains, promotion boundaries, readiness, forged-State rejection, both existing-builder acceptance paths, decision-visible mapping, frozen-Checkpoint behavior, strict persistence reconstruction/replay/fingerprints/lineage, optimistic saved/unchanged/conflict writes, canonical atomic files, execution counts, public/CLI compatibility, 62 Schemas, 77 generated outputs, and Package version `0.13.0`. | v0.14.0 internal foundation through private persistence implemented; end-to-end capture open | Yes |
| Private Session persistence and resume | skat-ai product | Not applicable | `supported` | `session_persistence_contracts.py` defines the immutable private version-1 document, Resume Result, Write Result, policies, and exact statuses. `session_persistence_codec.py` builds domain-separated canonical State/content fingerprints, strictly reconstructs State, Commands, Validation, Position Requests, and caller-supplied frozen Checkpoints, replays the accepted Log, verifies fingerprints, and recomputes current/ancestor/future/diverged lineage. `session_persistence.py` strictly loads private files and performs canonical optimistic expected-content-fingerprint writes through durable same-directory temporary files and atomic replacement. | One replay-valid `SessionStateV1`; optional canonical caller-supplied `SessionDecisionCheckpointV1` values from that Session; for file save, an existing parent, target path, and the expected current content fingerprint or null for an expected missing target. | This is a private internal file contract. It provides no Public API, Provenance, Schema, CLI, automatic Checkpoint collection, actual-card attachment, analysis execution, multi-process lock, directory creation, arbitrary recovery, or end-to-end Session workflow. | Retain `tests/test_session_persistence_contracts.py`, `tests/test_session_persistence_codec.py`, and `tests/test_session_persistence.py` coverage for exact constants and immutable fields, independent fingerprint oracles, same-revision corrections, checkpoint canonicalization, strict field and canonical reconstruction, tamper rejection, full replay, lineage recomputation, transition/export compatibility, canonical bytes, malformed files, expected-fingerprint status matrices, pre-replacement conflicts, fsync/replace failures, temporary-file cleanup, and unchanged public counts. | v0.14.0 bounded private persistence scope complete | Yes |
