...
and local/CI distribution gates. Installed CLI entry points, publication, and a
license decision remain open. Issue #138 adds the internal
...
Context, public redaction, and safe serialization. Workflow propagation, public
provenance integration, and adversarial end-to-end leakage enforcement remain
open.
...
| Requirement | Source | Rule section | Current status | Current implementation | Required input or information | Known limitation | Required validation or tests | Target milestone | Required before v1.0 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Stable public Python contract | skat-ai product | Not applicable | `partially_supported` | API contract version `1` defines `skat_ai.api.v1`, `skat_ai.errors`, exact Package/API exports, seven Root workflows, immutable Request/Result JSON documents, direct immutable execution options, public artifacts and flattened execution results, compatibility and version metadata, normal Result states, public errors and codes, Exit Codes, deprecation category, and legacy `main.CliUsageError` compatibility. The public facade validates, parses, and executes all seven workflows through internal Application orchestration, preserves Root output, injects validated Opponent Statistics, returns auxiliary artifacts separately, and performs no caller transport I/O. Package, API, Application, schema, provenance, and Domain versions remain independent. Setuptools Wheel and sdist artifacts include byte-identical private Schema resources, `py.typed`, and Package-Root `__version__`; artifact and separate clean-install checks execute the facade outside the repository. | One already loaded JSON-compatible Root object or immutable Request, an opaque caller input reference, matching direct workflow options, and optional paired Opponent Statistics document and reference. | The installable library distribution is implemented, but the installed CLI, broader Domain-error migration, public provenance exports, license decision, and publication remain absent. | Retain exact namespace/export snapshots, recursive immutability, lazy Package Resource schema behavior, RFC 6901 errors, all seven public executions, all Position/Historical submodes, all five Dataset operations, complete/unavailable preparation, lists, injection and artifact boundaries, output-validation controls, Application/legacy parity, no-I/O behavior, normal states, stable error translation, schema parity, Wheel/sdist and clean-install validation, no installed CLI or provenance, unchanged schemas/examples, and 70-output regressions. | v0.13.0 executable facade and installable library distributions implemented; installed CLI remains open | Yes |
| Field-level information provenance | skat-ai product | Not applicable | `partially_supported` | Internal contract version `1` defines canonical RFC 6901 paths, frozen slotted source references, field/subtree entries, exemptions, complete/partial-legacy/unavailable sidecar ledgers, deterministic JSON-leaf coverage summaries, same-ledger dependency and cycle validation, coarse temporal monotonicity, Information Use Context, context-use validation, engine-private public redaction, safe deterministic serialization, and explicit Confidence separation. Existing specialized provenance remains unchanged. | One JSON-compatible document, a matching sidecar ledger, and an Information Use Context where use is validated. Existing workflows do not construct these values yet. | No live, Multi-Step, Policy Comparison, Search, Historical Review, Replay Coaching, Dataset, list, or Opponent workflow propagates the ledger. There is no public provenance API export, schema, output field, CLI presentation, or adversarial workflow-level leakage enforcement. | Retain exact constants and canonical orders, every JSON Pointer case, immutability, source and Player identities, hard derivation and temporal rules, status and exemption relationships, leaf and coverage audits, dependency cycles, every context visibility/availability rule, non-disclosing redaction, safe serialization, Confidence separation, specialized-contract compatibility, no public provenance exports, unchanged schemas/CLI, and 70 generated outputs. | v0.13.0 contract foundation implemented; propagation remains open | Yes |
| Live information boundaries | skat-ai product | Not applicable | `partially_supported` | Existing live enforcement is unchanged. Declared Ouvert and both continuation branches authorize only their exact current public hands. Accepted 4.4.5 proof hands remain private; other hands, hidden skat, future cards, and proof state remain unavailable. The internal field-provenance language is available for later propagation. | Explicit mode, visibility, end reason, perspective, verifiable public history, valid declared-Ouvert exact hand, validated continuation or exact post-game adjudication, or normal-play historical replay. | General position input still trusts supplied context, and no workflow constructs or propagates a field-level ledger. | Retain rejection/redaction, declared-Ouvert and both continuation public-hand exceptions, hidden proof evidence, future-information, post-game leakage, and field-provenance contract-isolation tests. | v1.0 | Yes |
...
| Release and regression checks | skat-ai product | Not applicable | `supported` | `scripts/check.ps1` and GitHub Actions run Ruff, packaged-schema parity, input schema validation, generated-output validation, one Wheel and one sdist build with archive and separate clean-install validation, and pytest on Python 3.13. | Development dependencies and supported platform tooling. | Passing checks proves tested behavior, not complete ISkO/SkWO compliance; artifacts are validated but not published. | Require clean local and CI checks, synchronized docs/schemas, clean distribution validation, and human-controlled publication and release actions. | v1.0 | Yes |
