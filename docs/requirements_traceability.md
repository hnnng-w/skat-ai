...
Historical builder round trip, and immutable Request construction. Live Position
Request export, Decision checkpoints, Undo/correction, persistence, Public API,
Provenance, Schemas, CLI, examples/generated output, and UI remain open. Online-
...
| Interactive Session capture | skat-ai product | Not applicable | `partially_supported` | Issue #150 defines internal Session and Command version `1`, exactly three stable seated Players, Live and Retrospective Modes, phases, nine typed incremental Commands, an authoritative accepted Log, linear revision and promotion relationships, Diagnostics, Position/Historical readiness, valid-incomplete State, Transition Result semantics, and deterministic immutable serialization. Issue #151 adds transition and projection version `1`, canonical revision-zero creation, full accepted-Log replay, atomic application/rejection, revision-conflict precedence, monotonic phases, incremental Deal-through-end rule and information validation, exact/bounded ownership, trick/event/end derivation, promotion, readiness calculation, and forged-State detection. Issue #152 adds Session Request Export version `1`, immutable available/unavailable Results, one-replay Historical readiness gating, exact Retrospective mapping through the existing Historical builder and canonical round trip, and immutable existing Request construction without execution. | Caller-supplied Session identity, three stable Players and seats, Capture Mode, local Player for Live, and typed Commands with expected revisions; Historical export requires a fully ready Retrospective State with exact Deal, Declaration, legal Play prefix, Game ID, and supported ending facts. | No Live Position Request export, Decision checkpoints, end-to-end capture workflow, Undo, correction, persistence, Public API, Session Provenance, Schema, CLI Session Assistant, example, generated output, or UI exists. | Retain exact constants/policies, constructors, recursive immutability, deterministic projection/State/export serialization, conflict-before-semantics, atomic unchanged-State rejection, Live information restrictions, Retrospective ownership and Bedienpflicht, all declarations, continuations/endings and chains, promotion boundaries, readiness, forged-State rejection, builder acceptance, canonical record/serialization stability, replay/build/serialization counts, public/CLI compatibility, 62 Schemas, 77 generated outputs, and Package version `0.13.0`. | v0.14.0 Historical export foundation implemented; end-to-end capture open | Yes |
