...
| Final settlement | ISkO | 2.5.1-2.5.11; 3.6.1-3.6.4 | `partially_supported` | `final_settlement.py` covers wins, doubled losses, achieved/announced levels, Null variants, and supported Suit/Grand overbids from canonical declarations. | Complete result, valid declaration, game value, bid, and reliable trick ownership where required. | Source and output explicitly call the logic simplified; impossible Null, claims, concessions, and rule violations leave compliance gaps. | Build a normative contract/level/outcome table and settlement regression suite for every supported variant. | v1.0 | Yes |
