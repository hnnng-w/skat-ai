...
Result semantics. Command application and executable capture remain open, as do
export, Undo, persistence, Public API, Provenance, Schemas, CLI, and UI. Online-
...
| Interactive Session capture | skat-ai product | Not applicable | `partially_supported` | Issue #150 defines internal Session and Command version `1`, exactly three stable seated Players, Live and Retrospective Modes, phases, nine typed incremental Commands, an authoritative accepted Log, linear revision and promotion relationships, Diagnostics, Position/Historical readiness, valid-incomplete State, Transition Result semantics, and deterministic immutable serialization. | Caller-supplied Session identity, three stable Players and seats, Capture Mode, local Player for Live, current phase/revision, accepted Commands, and validated readiness values. | No Command application, phase advancement, incremental rule enforcement, capture workflow, Position/Historical export, Decision checkpoints, Undo, correction, persistence, Public API, Session Provenance, Schema, CLI Session Assistant, example, generated output, or UI exists. | Retain exact constants and policies, all constructors and relationships, recursive immutability, serialization, Live structural hand protection, public/CLI compatibility, 62 Schemas, 77 generated outputs, and Package version `0.13.0`; add stateful transition tests in Issue #151. | v0.14.0 foundation implemented; executable capture open | Yes |
