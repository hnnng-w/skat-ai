...
Result provenance, including base Historical execution. Public provenance
...
| Field-level information provenance | skat-ai product | Not applicable | `partially_supported` | Internal contract version `1` defines canonical RFC 6901 paths, immutable ledgers, coverage, dependencies, Information Use Context, redaction, serialization, and Confidence separation. Live, retrospective, Dataset, Preparation, Opponent, Profile, historical-list, and comparison propagation remain complete. Issue #146 adds complete non-legacy Position and Historical Root Result ledgers, base Historical result-only bundles, forward-only Declaration/Value/Overbid/score/Result/Settlement/Performance/list/replay/event dependencies, and private-proof-safe redaction. | A supported Application invocation, its exact retained decision/retrospective/aggregate stages and serialized Result, and matching internal sidecars. | Public provenance integration remains absent. Existing specialized Confidence and source-provenance contracts remain separate. | Retain core contract tests plus complete all-seven-workflow Root coverage, all Search statuses and terminal/event combinations, split and audit isolation, progression prefix safety, stage/context denial, actual/future/outcome and cross-domain isolation, private-root invariance, redaction, deterministic ordering, unchanged call counts, no public exports, unchanged schemas/CLI, and 70 generated outputs. | v0.13.0 internal Result propagation complete; public propagation remains open | Yes |
| Live information boundaries | skat-ai product | Not applicable | `partially_supported` | Existing broad enforcement remains, and live Position Application execution validates complete pre-selection decision ledgers and attaches a complete non-legacy exact Result ledger. Declared Ouvert and both continuation branches authorize only their exact current public hands. Accepted 4.4.5 proof hands, other hands, hidden skat, future cards, private worlds, and derived seeds remain unavailable. Retrospective collectors preserve the same decision-time boundary before adding observed-card assessments. | Explicit mode, local perspective, verifiable public history, valid declared-Ouvert or continuation public hands, effective seed-free settings, and the exact Application Result. | General position input still trusts supplied context, and public provenance integration remains open. | Retain decision-time context enforcement, all-leaf accounting, dependency rejection/redaction, Ouvert and continuation exceptions, hidden proof evidence, post-game and private-world leakage, retrospective stage isolation, determinism, call-count, and public-boundary tests. | v1.0 | Yes |
| Retrospective information | skat-ai product | Not applicable | `supported` | Post-game mode permits known skat and ended states; every supported historical terminal record reconstructs each actual decision from its decision-time snapshot, including exact declared-Ouvert and continuation ownership without future leakage. Internal retained-stage provenance separates decision input and analysis from actual-card assessment in flat Position, Historical Immediate/Search Review, and Replay Coaching, with final Outcome Context restricted to the complete report. Complete Historical Root Result provenance covers the canonical post-game record, replay, points, events, terminal adjudication, and Settlement without making those facts decision-time evidence. | Post-game mode and supplied retrospective facts, or a validated historical snapshot, plus an internal Application provenance bundle when field-level auditing is required. | Public provenance output remains separate work. | Retain complete-history retrospective facts separately from live facts, 0-30 decision cardinality, continuation boundaries, actual/future/outcome isolation, redaction, and unchanged public output tests. | v1.0 bounded requirement complete | Yes |
