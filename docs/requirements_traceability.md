...
generated scenario, browser transport, and UI remain absent. `v1.0.0` remains
unready after this focused milestone work.
...
| Requirement | Source | Rule section | Current status | Current implementation | Required input or information | Known limitation | Required validation or tests | Target milestone | Required before v1.0 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Match Capture identity and metadata | skat-ai product | Not applicable | `partially_supported` | Issue #160 defines internal immutable version-1 Match Capture, Source Metadata, Media Timecode, Tournament Format, Participant, and Player Statistics Snapshot values. The append-only registry executes only `euroskat_36_standard_v1` with provider EuroSkat, display name 36er Standard, three Players, and 36 Games. Source metadata distinguishes game platform from descriptive YouTube/other-video/manual evidence; exactly three fixed-place participants, unique identities, one perspective Player, optional existing Opponent Statistics records, same-instant snapshot time, and deterministic defensive serialization are enforced. Issue #163 retains the definition inside one exact Workspace, and Issue #164 consumes it through internal Capture Application services. | Caller-supplied Match identity/title/platform, optional external ID and played time, one canonical format, one descriptive source, exactly three participants, one perspective Player, and optional validated statistics snapshots. | Observed Games/commentary, Workspace persistence, and rapid-entry services are separate internal layers. Materialization, Public API, CLI, Schema, examples, generated scenarios, browser UI, YouTube integration, and EuroSkat integration remain absent. Ranking, qualification, prize, fee, and bonus rules are not implemented. | Retain source relationships and no-network behavior, exact registry identity/order, snapshot identity/time reconciliation, fixed-place and perspective relationships, uniqueness, immutability, fresh serialization, unchanged public exports, and unchanged Package/Schema/scenario counts. | v0.15.0 metadata foundation complete; usable capture open | Yes |
| Observed Game evidence and commentary | skat-ai product | Not applicable | `partially_supported` | Issue #161 defines internal immutable version-1 observed Game, Play, evidence-summary, free-text Decision commentary, and linked later-response values. One builder derives exact Match linkage and perspective, validates canonical historical seats, optional perspective hand/original Skat/Discards, Game and Decision timecodes, public Turn Order, winner-led progression, bounded partial perspective ownership and legal play, exact complete Suit/Grand/Null replay, complete-card reconciliation, canonical annotation order, and deterministic defensive serialization. Issue #163 places partial or complete records in exact rotated Slots without hidden completion. Issue #164 performs incremental updates only by rebuilding complete candidates through that same builder. | One exact Match definition, Match position, exact Match Player seat order, optional observed Card evidence, optional Declarer/Declaration, zero through 30 public Plays, and optional commentary/response annotations. | Workspace persistence and internal rapid entry exist, but Historical/Session/Dataset/list/report materialization, Public API, CLI, Schema, example, generated scenario, browser UI, and tactical interpretation remain absent. Missing Skat and Discards remain unknown. | Retain null-versus-empty semantics, incomplete-trace information safety, complete legal replay, Skat/Discard overlap, all evidence-capability relationships, free text and multiline preservation, commentator forms, response direction, canonical ordering, immutability, fresh serialization, and unchanged public/count boundaries. | v0.15.0 observed single-Game foundation complete; usable capture open | Yes |
| Persistent EuroSkat Match Workspace | skat-ai product | Not applicable | `partially_supported` | Issue #163 defines internal immutable version-1 Workspaces with exactly 36 empty, observed-Game, or passed-deal Slots; existing fixed-list Dealer/seat rotation over twelve rounds; unique Game IDs; global time ordering; revisioned set/pass/clear/definition-correction operations; evidence-derived Progress; domain-separated Workspace/content fingerprints; strict nested Resume; canonical private Load; and optimistic same-directory atomic Save. Issue #164 wraps the unchanged operations and derives non-persisted Views and Progress. | One exact `euroskat_36_standard_v1` Match definition, optional partial observed Games or passed deals, expected revisions, and an explicit caller file path/fingerprint for Save. | Structural completion does not imply evidence completion. No Capture autosave orchestration, materialization, opponent-statistics application, Public API, Match Schema, CLI, browser server, UI, distributed lock, retry/merge, cloud, encryption, or backup policy exists. | Retain exact 36-Slot/rotation/revision/timecode rules, partial-evidence safety, Progress counts, both fingerprint domains, strict reconstruction, canonical file bytes, optimistic conflict and atomic-failure preservation, private-data retention, Session/fixed-list compatibility, and unchanged public/count boundaries. | v0.15.0 Workspace/persistence foundation complete; usable capture open | Yes |
| Rapid post-game Match Capture Application services | skat-ai product | Not applicable | `partially_supported` | Issue #164 defines internal version-1 transport-free Card entries, Position Views, Application Results, deterministic Game and annotation IDs, setup updates, automatic Player/Decision Play append, Play truncation with annotation cleanup, Commentary/Response Link editing, and Passed Deal/clear wrappers. Exact perspective legal Cards are separated from bounded observation candidates under `no_hidden_completion`. | One already loaded exact Workspace, target Match position, expected revision, and operation-specific caller evidence. | Services perform no file/network I/O, autosave, Public API, Match Schema, CLI, browser transport/UI, Player Statistics application, materialization, analysis, tactical interpretation, or AI commentary. Bounded candidates do not assert ownership or legality. | Retain exact versions/tuples/policies, all six states and blocker order, exact/bounded selection, deterministic IDs, conflict precedence, one-revision atomic batches, annotation cleanup, View/Progress reconciliation, no hidden completion, no execution/import boundary violations, unchanged persistence bytes, and unchanged public/count/version boundaries. | v0.15.0 rapid-entry foundation complete; usable capture open | Yes |
...
| Release and regression checks | skat-ai product | Not applicable | `supported` | `scripts/check.ps1` and GitHub Actions run Ruff, 63-Schema packaged parity, Root and Session example validation, 85-scenario generated-output validation, Wheel/sdist archive and clean-install API/installed/module Root and Session CLI validation, and pytest on Python 3.13. | Development dependencies and supported platform tooling. | Passing checks proves tested behavior, not complete ISkO/SkWO compliance; distribution artifacts are validated but no Package-index publication is claimed. | Require clean local and CI checks, synchronized docs/schemas, exact entry metadata, installed/module/Legacy/API parity, and human-controlled publication and Release actions. | v1.0 | Yes |
