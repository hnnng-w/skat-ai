...
| Historical-game representation | skat-ai product | Not applicable | `partially_supported` | A separate version-1 workflow preserves stable game/player IDs, fixed seats, all initial cards, final declaration, pickup/discards or Hand state, and all 30 plays for normal completion; it derives winners, points, matadors, result, overbid, and settlement and round-trips canonically. | Complete `historical_game_input` record ending with `normal_completion`. | Full auction events, claims/concessions and other end reasons, disputes, per-decision information snapshots, replay analysis, and training wrappers are absent. | Retain schema/runtime, identity, deal, ownership, legality, round-trip, CLI, example, output, and settlement tests; add later approved end reasons separately. | v1.0 | Yes |
