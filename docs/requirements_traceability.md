...
legacy Root ledgers. Complete non-legacy Position/base Historical Result and
public provenance integration remain open.
...
and `--quiet`; Plan and CLI output are card-free while a complete nested dataset
...
| Requirement | Source | Rule section | Current status | Current implementation | Required input or information | Known limitation | Required validation or tests | Target milestone | Required before v1.0 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Stable public Python and installed CLI contract | skat-ai product | Not applicable | `supported` | API contract version `1` defines `skat_ai.api.v1`, `skat_ai.errors`, exact Package/API exports, seven Root workflows, immutable Request/Result JSON documents, direct immutable execution options, public artifacts and flattened execution results, compatibility and version metadata, normal Result states, public errors and codes, Exit Codes, deprecation category, and legacy `main.CliUsageError` compatibility. The public facade validates, parses, and executes all seven workflows through internal Application orchestration without caller transport I/O. Installed CLI contract version `1` adds exactly `skat-ai`, `python -m skat_ai`, one canonical Package implementation, invocation-specific help, `--version`, and Legacy `python main.py` compatibility. Package, API, Application, CLI, Schema, Provenance, and Domain versions remain independent. Wheel and sdist artifacts include byte-identical private Schema resources, `py.typed`, Package-Root `__version__`, exact Console Script metadata, and the module entry point. | A Root JSON file and optional CLI transport paths, or one already loaded JSON-compatible Root object or immutable Request with matching options and optional paired Opponent Statistics. | Broader Domain-error migration, public Provenance exports, license selection, and publication remain absent. Installed examples and interactive sessions are not provided. | Retain exact namespace/export snapshots, parser/help/version contracts, all seven workflows and representative submodes across installed/module/Legacy/Application/Public API boundaries, JSON and human output, quiet and export behavior, semantic and expected failures, normal unavailable states, Legacy names and patch points, schema parity, exact Wheel/sdist entry metadata, both clean-installed CLI forms, no Provenance, unchanged examples, and 70-output regressions. | v0.13.0 foundation implemented | Yes |
| Field-level information provenance | skat-ai product | Not applicable | `partially_supported` | Internal contract version `1` defines canonical RFC 6901 paths, immutable ledgers, coverage, dependencies, Information Use Context, redaction, serialization, and Confidence separation. Application and live-analysis provenance version `1` add complete flat and simulated live decision ledgers plus an all-leaf partial-legacy Position Result ledger. Retrospective-review and Replay Coaching provenance version `1` separate flat and Historical decision-time input/analysis from actual-card assessment, cover requested review and complete Coaching summaries, and add selected partial-legacy Position/Historical Result entries. Issue #145 adds complete Dataset, Preparation, Opponent, Profile, historical-list, comparison, and exact Root workflow ledgers. | A supported Application invocation, its exact retained decision/retrospective/aggregate stages and serialized Result, and matching internal sidecars. | Complete non-legacy Position/base Historical Result and public provenance integration remain absent. Existing specialized Confidence and source-provenance contracts remain separate. | Retain core contract tests plus complete live/retrospective/Dataset/list/opponent coverage, all Search statuses, split and audit isolation, progression prefix safety, stage/context denial, actual/future/outcome isolation, private-root invariance, redaction, deterministic ordering, unchanged call counts, no public exports, unchanged schemas/CLI, and 70 generated outputs. | v0.13.0 internal workflow propagation implemented; Result/public propagation remains open | Yes |
| Live information boundaries | skat-ai product | Not applicable | `partially_supported` | Existing broad enforcement remains, and live Position Application execution now validates complete pre-selection decision ledgers and attaches a complete-coverage partial-legacy Result ledger. Declared Ouvert and both continuation branches authorize only their exact current public hands. Accepted 4.4.5 proof hands, other hands, hidden skat, future cards, private worlds, and derived seeds remain unavailable. Retrospective collectors preserve the same decision-time boundary before adding observed-card assessments. | Explicit mode, local perspective, verifiable public history, valid declared-Ouvert or continuation public hands, effective seed-free settings, and the exact Application Result. | General position input still trusts supplied context; public provenance integration remains open, and out-of-scope Result branches remain legacy exemptions. | Retain decision-time context enforcement, all-leaf accounting, rejection/redaction, Ouvert and continuation exceptions, hidden proof evidence, post-game and private-world leakage, retrospective stage isolation, determinism, call-count, and public-boundary tests. | v1.0 | Yes |
| Retrospective information | skat-ai product | Not applicable | `supported` | Post-game mode permits known skat and ended states; every supported historical terminal record reconstructs each actual decision from its decision-time snapshot, including exact declared-Ouvert and continuation ownership without future leakage. Internal retained-stage provenance separates decision input and analysis from actual-card assessment in flat Position, Historical Immediate/Search Review, and Replay Coaching, with final Outcome Context restricted to the complete report. | Post-game mode and supplied retrospective facts, or a validated historical snapshot, plus an internal Application provenance bundle when field-level auditing is required. | Public provenance output and complete non-legacy Root Result provenance remain separate requirements. | Retain complete-history retrospective facts separately from live facts, 0-30 decision cardinality, continuation boundaries, actual/future/outcome isolation, redaction, and unchanged public output tests. | v1.0 bounded requirement complete | Yes |
...
| Release and regression checks | skat-ai product | Not applicable | `supported` | `scripts/check.ps1` and GitHub Actions run Ruff, packaged-schema parity, input schema validation, generated-output validation, one Wheel and one sdist build with archive and separate clean-install API/installed/module CLI validation, and pytest on Python 3.13. | Development dependencies and supported platform tooling. | Passing checks proves tested behavior, not complete ISkO/SkWO compliance; artifacts are validated but not published. | Require clean local and CI checks, synchronized docs/schemas, exact Console Script and module-entry metadata, both clean-installed command forms, CLI/API/Legacy parity, and human-controlled publication and release actions. | v1.0 | Yes |
