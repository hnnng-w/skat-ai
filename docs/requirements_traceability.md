...
proof or Runtime execution. All other current Claim boundaries remain
`not_supported_v1`. Candidate directions include proof, adjudication, Historical
...
| Claims | ISkO | 4.4.4-4.4.6; bounded 4.1.3-4.1.6 | `partially_supported` | Flat positions support both 4.4.4 declarer-exposure branches, bounded exact final 4.4.5 defender open play, non-adjudicating 4.4.5/4.1.6 continuation, and final 4.4.6 open-card throw for either party. Historical games support all five terminal shortenings, timed non-terminal continuation for either public-hand branch, and the bounded sequence of at most one continuation before one terminal shortening. Existing terminal adjudicators remain authoritative. Matrix version `2` preserves those cases. Issue #183 adds private version-1 structured party-wide all-remaining-Tricks Claim, complete Evidence, exact-state, Proof Request/preparation, assignment, diagnostic-line, and Result contracts without execution. | Existing Runtime paths require concrete parties, final declaration/value, complete applicable public current or thrown hand or exact three-hand adjudication evidence, reliable ownership, hand size, current Trick, turn, and Skat reconciliation. The private Claim contracts require one exact claimant Player and party, the complete Deal, exact legal play prefix and current Trick, complete remaining hands, and Retrospective-only complete-world evidence. | The approved Claim has no proof executor, adjudication, Runtime/Historical ending, Settlement integration, or public exposure yet. Specific future-Trick Claims, generalized non-jack exclusion, generalized correction, free text, natural language, simultaneous throws, arbitrary streams, unlimited proof, generative adjudication, unclassified conduct, multiple non-terminal events, and defender proof beyond five unresolved Tricks are `not_supported_v1`. | Retain existing Runtime behavior and add private contract/version/vocabulary, exact replay/state reconciliation, five-Trick preparation, supplied Result invariant, no-traversal, matrix identity/status-group, Generic Search separation, legacy, Runtime/Historical coverage, and compatibility tests. | v1.0 bounded approved Claim proof and integration remain open | Yes |
...
legacy compatibility, one approved non-executable implementation direction, and
...
Private contracts and exact-state preparation exist, while the Matrix Runtime
module tuple remains empty until proof and adjudication work. Invalid or
unavailable proof creates no terminal outcome or Settlement. Every other current Claim boundary is
...
* ISkO 4.4.4-4.4.6 defines specific open-card shortcuts. It does not define a
  ...
  state preparation; proof execution, adjudication, Runtime/Historical ending,
