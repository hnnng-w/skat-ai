...
classification. Corpus persistence/import/mutation, a Player Catalog, multi-
Match Player Snapshot history, Commentary/Response export, target separation,
Dataset version `2`, annotations, cross-game summaries, evaluation baselines,
derived AI tags, browser workflows, and public exposure remain open. No
...
| Private Learning Corpus identity and Catalog | skat-ai product | Not applicable | `partially_supported` | Issue #171 defines immutable content-addressed Match Snapshots from exact strictly resumed Workspace persistence documents, exact Player Observations, observed-Game content fingerprints, Snapshot-scoped Game/Decision/Commentary/Response References, lightweight Catalog entries, explicit current selections, and non-mutating duplicate/revision classification. | One exact in-memory `MatchWorkspacePersistenceDocumentV1`, stable caller-supplied Corpus ID/revision, lightweight entries, and explicit same-Match current selections. | Identity and in-memory Catalog contracts only. No Corpus persistence/import/mutation, deletion, Player Catalog, cross-revision lineage, report import, Commentary export, Dataset version `2`, browser/CLI/API/Schema, or model training exists. | Retain exact versions/policies/domains, strict Resume/equality, source fingerprints, immutable Workspace copy, canonical order, closed references, case-sensitive Player IDs, counts, selection invariants, classification precedence, no-I/O/no-analysis boundaries, and unchanged 0.15.0/63/6/85 compatibility. | v0.16.0 identity foundation complete; remaining Corpus layers open | No |
