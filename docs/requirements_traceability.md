...
stable boundary errors. Packaging, installed CLI entry points, Package Resource
schemas, typing metadata, and broader distribution work remain open. Issue #138 adds the internal
...
| Requirement | Source | Rule section | Current status | Current implementation | Required input or information | Known limitation | Required validation or tests | Target milestone | Required before v1.0 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Stable public Python contract | skat-ai product | Not applicable | `partially_supported` | API contract version `1` defines `skat_ai.api.v1`, `skat_ai.errors`, exact Package/API exports, seven Root workflows, immutable Request/Result JSON documents, direct immutable execution options, public artifacts and flattened execution results, compatibility and version metadata, normal Result states, public errors and codes, Exit Codes, deprecation category, and legacy `main.CliUsageError` compatibility. The public facade validates, parses, and executes all seven workflows through internal Application orchestration, preserves Root output, injects validated Opponent Statistics, returns auxiliary artifacts separately, and performs no caller transport I/O. Package, API, Application, schema, provenance, and Domain versions remain independent. | One already loaded JSON-compatible Root object or immutable Request, an opaque caller input reference, matching direct workflow options, and optional paired Opponent Statistics document and reference. | The current schema backend requires a source or editable checkout. Package Resource schemas, distribution builds, installed CLI, typing metadata, broader Domain-error migration, and public provenance exports are absent. | Retain exact namespace/export snapshots, recursive immutability, lazy import and schema behavior, RFC 6901 errors, all seven public executions, all Position/Historical submodes, all five Dataset operations, complete/unavailable preparation, lists, injection and artifact boundaries, output-validation controls, Application/legacy parity, no-I/O behavior, normal states, stable error translation, no packaging or provenance, unchanged schemas/examples, and 70-output regressions. | v0.13.0 executable public facade implemented; stable installed distribution remains open | Yes |
