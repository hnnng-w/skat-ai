...
communication data`. Functional Issues #171 through #179 add the private Learning
Corpus, Dataset-v2, partition, Summary, and local browser workflow. Issue #180
changes only Package version, current version expectations, Changelog, and
Release-state documentation to complete Release preparation. Package version
`0.16.0` preserves Python `>=3.13`, Public API contract version `1`, seven Root
workflows, 63 authoritative and packaged Schemas, six Session examples, and 85
generated outputs, and passes 6,925 pytest tests. Publication remains open and
manual; `v0.16.0` is not yet tagged or published.
...
canonical downloads. Issue #180 prepares Package version `0.16.0` and current
Release documentation without product behavior changes; publication remains
open. Derived persistence, Public APIs/Schemas, database/
