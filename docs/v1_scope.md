...
collection, end-to-end capture, and UI remain open. Issue #156 adds stable
...
clean-install validation. Online-
...
| Public Session API | Stable transport-free `skat_ai.api.v1.session` version `1` with exact immutable Session type identity, strict Command parsing, ten one-call operations, typed Results, and in-memory persistence construction/resume. | It adds no Session Root workflow, analysis execution, public file Save/Load, CLI command, automatic Checkpoint collection, actual-card attachment, example/generated output, or UI. |
...
| Interactive Session foundation | Immutable internal authoring State with exactly three stable seated Players, Live or Retrospective Mode, phase, accepted typed Command Log, linear revision, validation Diagnostics, Position/Historical readiness, deterministic full replay, frozen projection, atomic incremental Command application, immutable available/unavailable canonical Historical and Position Request export, and frozen local pre-Play Decision Checkpoints. | Issues #150 through #153 define and execute the internal transition, no-execution Request-export, declared-Ouvert public-hand, and Checkpoint boundaries. Internal history editing and private persistence are defined by the following rows; Public API, Provenance, Schema, CLI, automatic Checkpoint collection, end-to-end capture, and UI remain absent. |
| Session History Edit and Checkpoint lineage | Immutable internal version-1 strict-prefix Undo, one-command replacement with deterministic original-suffix replay, valid partial corrected States, and exact Checkpoint relationships. | Issue #154 implements the internal boundary. It adds no automatic Redo stack, merging, arbitrary Log surgery, public API, Schema, CLI, or workflow execution. |
| Private Session persistence and resume | Immutable private version-1 document containing the authoritative accepted-Log State and canonically ordered caller-supplied frozen Decision Checkpoints, with deterministic State/content fingerprints, strict reconstruction/replay, and recomputed lineage. Canonical file writes use optimistic expected-content-fingerprint `saved`, `unchanged`, or `conflict` outcomes and atomic same-directory replacement. | Issue #155 implements only this private internal boundary. It adds no Public API, Provenance, Schema, CLI, automatic Checkpoint collection, actual-card attachment, Session-triggered analysis, or end-to-end capture workflow. See [Session persistence and resume](session_persistence_and_resume.md). |
...
* Support interactive live and retrospective input and Session capture. Issue
  ...
  files, and atomic same-directory replacement. Session-triggered analysis,
  actual-card Checkpoint attachment, Public API, Provenance, Schemas, CLI,
  examples/generated output, automatic Checkpoint collection, end-to-end
  capture, and UI remain open.
...
Issue #156 supersedes older rows below that call Public Session API, Session
Provenance, or Session Schemas absent. The active package gate now validates 63
Schemas and the clean in-memory Session facade; the published `v0.13.0` baseline
remains 62 Schemas and 77 generated outputs. End-to-end Session capture remains
open because analysis triggering, automatic/actual-card Checkpoints, public file
transport, CLI, examples/generated outputs, and UI are still absent.
...
| Public Session interface | The stable 52-name Session subnamespace preserves exact existing immutable type identity; Commands, operation/value pairs, persistence mappings, optional provenance, and complete Results validate through the standalone packaged Session Schema. Public wrappers invoke one existing internal operation and perform no transport I/O or analysis. |
...
| Interactive input and session capture | Internal Session and Command version `1` define exactly three stable seated Players, Live/Retrospective Modes, phases, typed Commands, authoritative accepted linear revisions, Diagnostics, readiness, valid-incomplete State, Transition Result semantics, and deterministic serialization. Transition and projection version `1` add canonical revision-zero creation, full accepted-Log replay, atomic application/rejection, monotonic phase advancement, incremental Deal-through-end rule and information validation, exact/bounded ownership, trick/event/end derivation, promotion, export-readiness calculation, forged-State rejection, determinism, and execution-count tests. Session Request Export version `1` adds immutable normal Results, one-replay Historical readiness gating, exact canonical mapping and builder round trip, all supported ending/event chains, immutable existing Request construction, and no-execution/public-boundary tests. Issue #153 adds immutable Position Export Options, one-replay information-safe Position mapping through the existing builder, one narrow declared-Ouvert public-hand Command with owner-aware coexistence/shrinking, and immutable replay-verified pre-Play Decision Checkpoints. Issue #154 adds strict-prefix Undo, one-command correction, deterministic first-rejection suffix replay, valid partial corrected States, and exact Checkpoint lineage. Issue #155 adds private deterministic persistence/resume with strict State and Checkpoint reconstruction, accepted-Log replay, fingerprint verification, lineage recomputation, and optimistic atomic file writes. Completion still requires Session-triggered analysis, actual-card Checkpoint attachment, Public API, Provenance, Schemas, CLI Session Assistant, examples/generated output, automatic Checkpoint collection, end-to-end live/retrospective capture, and UI. |
...
| Session history editing | Internal version-1 contracts and behavior provide four exact Undo statuses, five exact Correction statuses, four exact Checkpoint relationships, revision-conflict precedence, projection-level strict-prefix reconstruction, exact removed/replayed/discarded source suffixes, valid partial corrected States, immutable inputs, and bounded deterministic replay. Public exposure, an automatic Redo stack, arbitrary Log surgery, and automatic Checkpoint collection remain open. |
| Private Session persistence and resume | `session_persistence_contracts.py`, `session_persistence_codec.py`, and `session_persistence.py` provide the immutable private version-1 document and Results, domain-separated canonical State/content fingerprints, exact typed reconstruction, accepted-Log replay and fingerprint verification, caller-supplied frozen Checkpoint canonicalization, recomputed current/ancestor/future/diverged lineage, strict loading, canonical UTF-8 file writes, expected-content-fingerprint `saved`/`unchanged`/`conflict` outcomes, a second pre-replacement check, and durable atomic same-directory replacement. Focused tests in `tests/test_session_persistence_contracts.py`, `tests/test_session_persistence_codec.py`, and `tests/test_session_persistence.py` cover deterministic identity, same-revision corrections, malformed/tampered files, lineage, optimistic conflicts, canonical bytes, and replacement failures. Public API, Provenance, Schema, CLI, automatic Checkpoint collection, actual-card attachment, Session-triggered analysis, and end-to-end capture remain open. |
