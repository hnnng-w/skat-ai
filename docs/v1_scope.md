...
general Opponent, complete non-legacy Result, and public integration remain open.
...
* Enforce field-level live-information provenance across inputs, analysis,
  ...
  Dataset, list, general Opponent, complete non-legacy Result, and public
...
| Stable installed interface | API contract version `1` provides documented public namespaces, additive exact exports, workflows, immutable JSON documents, direct execution options, public results and artifacts, compatibility/version metadata, normal Result states, stable errors/codes, and legacy Root CLI compatibility. The public facade validates, parses, and executes all seven Root workflows through internal Application orchestration without caller transport I/O or a dependency on repository-root `main.py`. Explicit Setuptools metadata, private byte-identical Package Resource schemas, `py.typed`, Package `__version__`, Wheel/sdist artifacts, archive inspection, and separate clean-install external-working-directory tests are implemented. Installed CLI contract version `1` adds exactly `skat-ai = skat_ai.cli:main`, `python -m skat_ai`, a canonical Package parser/transport, `--version`, invocation-specific help, unchanged JSON/output/error/Exit Code behavior, and Legacy `python main.py` compatibility. Wheel and sdist environments verify metadata, both installed forms, unavailable and error states, and exact Public API parity. No publication is implied. |
...
| Documentation | README, installed CLI, packaging, architecture, input/output, scoring, game-end, overbid, performance, examples, schema validation, roadmap, handoff, traceability, and scope documentation agree with behavior, rule ownership, stable fields, limitations, Python baseline, and release baseline. |
