...
duplicate/revision classification. Corpus persistence/import, a Player Catalog,
multi-Match Player Snapshot history, Commentary and Response export, separate
behavior/strategy/communication targets, Dataset version `2`, annotations,
cross-game summaries, evaluation baselines, derived AI tags, browser workflows,
and public exposure remain open. No production model is planned.
...
| Private Learning Corpus identity and Catalog | Internal immutable content-addressed Match Snapshots built only from exact strictly resumed Workspace persistence documents, with exact Player Observations, observed-Game fingerprints, Snapshot-scoped closed Game/Decision/Commentary/Response References, lightweight Catalog entries, explicit current selections, and duplicate/revision classification. | Issue #171 adds identity and in-memory contracts only. Match Workspaces remain editable authority; future analysis/annotation artifacts and Dataset version `2` remain separate. No Corpus persistence/import, mutation, Player Catalog, report import, Commentary export, browser, CLI, Public API, Schema, or model training exists. See [Learning Corpus identity and Catalogs](learning_corpus_identity_and_catalogs.md). |
...
* Support usable manual post-game Match capture for one EuroSkat 36er Standard
  ...
  local downloads, completing the functional local milestone. Issue #171
  subsequently adds a separate private immutable Learning Corpus Snapshot/
  reference and lightweight Catalog identity foundation without changing
  Workspace authority. Public Match API,
...
| --- | --- |
| Match capture | Issues #160 through #167 retain exact internal metadata, observed Games, 36-Slot Workspace/persistence, rapid-entry services, private no-JSON browser/autosave transport, Match-bound Statistics, information-safe Decision preparation, strict Historical and unpartitioned Training-source materialization, and complete fixed-list aggregation. Issue #168 adds explicit one-Decision Immediate/Search/Auto and strict Historical execution through one existing Application invocation, existing-behavior eligible Profile application, no-workflow materialization, deterministic max-eight ephemeral reports, concurrency invalidation, and authenticated canonical downloads. The functional local `v0.15.0` Package baseline is published at commit `ec1c154`. Issue #171 adds private content-addressed Learning Corpus Match Snapshot/reference and lightweight Catalog identity contracts only. Corpus persistence/import/mutation, Public Match API, Match Schema/data workflow, global Player Catalog, communication-aware Dataset work, and database/remote deployment remain open; YouTube and EuroSkat integration are not required before v1.0. |
