...
and readiness. Engine Request export, Decision checkpoints, Undo/correction,
persistence, Public API, Provenance propagation, Schemas, CLI, examples/generated
output, and UI remain open. Online-platform adapters, browser extensions, and
website scraping remain outside this bounded milestone.
...
| Interactive Session foundation | Immutable internal authoring State with exactly three stable seated Players, Live or Retrospective Mode, phase, accepted typed Command Log, linear revision, validation Diagnostics, Position/Historical readiness, deterministic full replay, frozen projection, and atomic incremental Command application. | Issues #150 and #151 define and execute the internal transition boundary; they do not export Engine Requests, create Decision checkpoints, persist, Undo/correct, or expose a Public API, Provenance contract, Schema, CLI, or UI. |
...
* Support interactive live and retrospective input and Session capture. Issue
  ...
  forged-State detection. Engine Request export, Decision checkpoints, end-to-
  end capture, Undo/correction, persistence, Public API, Provenance, Schemas,
  CLI, examples/generated output, and UI remain open.
