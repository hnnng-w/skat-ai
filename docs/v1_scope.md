...
execution/export, Public Match API and Schema/data workflow, and actual Profile
application remain open. `v1.0.0` remains unready, and its final planning still
...
| Match Capture metadata | Internal immutable Match identity, descriptive media/manual source, reusable millisecond bounds, exact named format, three fixed-place participants, optional historical Opponent Statistics snapshots, and one perspective Match Player. | Issues #160 through #164 define and consume the value; Issue #165 provides private browser creation and bounded correction; Issue #166 adds Match-bound Snapshot editing and time-safe Profile preparation; Issue #167 consumes it without changing persistence. Public Match API/Schema/data workflow, global history, actual Profile application, YouTube/EuroSkat integration, ranking, qualification, and commercial rules remain absent. |
| Observed Game capture | Internal immutable Match-linked Game facts, exact historical seats, optional perspective hand/original Skat/Discards, zero through 30 public Plays, free-text commentary on any Player Decision, linked later responses, and deterministic evidence capabilities. | Issue #161 retains observations without hidden completion. Issue #163 stores those exact values, Issue #164 rebuilds complete candidates through the same builder, and Issue #167 derives internal preparation without writing hidden completion back. Public API/CLI/Schema, browser materialization, tactical interpretation, and workflow execution remain absent. |
| Private Match Workspace | Internal immutable exact 36-position EuroSkat Workspace with fixed rotation, revisions, Progress, fingerprints, strict Resume, and optimistic atomic local files. | Issue #165 adds one explicit-file locked autosave context and explicit conflict Reload. Issue #167 derives non-persisted materialization values. Structural completeness differs from evidence completeness and Historical materializability; no distributed lock, retry/merge, remote/cloud/encryption/backup, browser/public materialization, or Public Match API/Schema exists. |
| Match Capture Application services | Internal transport-free rapid-entry operations over one loaded Workspace, with UI-ready Views, exact/bounded Cards, automatic Player/Decision derivation, truncation cleanup, annotations, and revisioned Results. | Issue #165 directly composes these services without changing their no-I/O/no-hidden-completion contract. Issue #167 remains a separate preparation layer. Public Match API/Schema/data workflow, browser controls, analysis, and tactical interpretation remain absent. |
| Match review and materialization preparation | Internal evidence-aware acting-own-hand Decision snapshots, strict complete-Deal normal-completion Historical Games, unpartitioned Training source Records, and complete fixed-three-player list plus aggregation materialization. | Issue #167 preserves the actual Card as retrospective evidence, excludes future-opponent leakage, retains Skat/Ouvert semantics, prepares time-safe relative Profile bindings without application, uses Match-level `played_at`, preserves Passed Deals and Commentary sidecars, and executes no workflow. It is internal and adds no persistence field, Public Match API/Schema/export, browser action, Dataset Plan/partition/sample, Profile application, report, example, or generated scenario. See [Match review and materialization](match_review_and_materialization.md). |
| Local Match Capture interface | Internal version-1 Web, Protocol, and Capture CLI private transport with one explicit file, loopback token/same-origin server, no-JSON creation, 36-position UI, setup/Card/correction/annotation/Statistics forms, CAS autosave, explicit Reload, and packaged progressive assets. | It is not a Root workflow or Public API and adds no remote bind, account, encryption claim, external source integration, analysis, Profile application, materialization/export control, Schema, example, or generated scenario. Issue #167 is not browser-exposed. See [Local Match Capture interface](local_match_capture_interface.md) and [Match Player Statistics](match_player_statistics.md). |
...
* Support usable manual post-game Match capture for one EuroSkat 36er Standard
  ...
  external-lot behavior, without workflow execution. Browser execution/export,
...
| --- | --- |
| Match capture | Issues #160 through #164 retain exact internal metadata, observed-Game, 36-Slot Workspace/persistence, and transport-free rapid-entry contracts without hidden completion. Issue #165 adds the bounded private local no-JSON interface, exact/bounded Card presentation, correction/annotation forms, one-file CAS autosave, explicit Reload, loopback security, and installed/module/Legacy Capture CLI parity. Issue #166 adds editable Match-bound Statistics Snapshots, strict temporal eligibility, canonical eligible input, and existing Profile derivation without application. Issue #167 adds internal information-safe Decision preparation, strict Historical and unpartitioned Training-source materialization, and complete existing fixed-list construction/aggregation with no execution. Completion still requires explicit analysis execution, browser controls and public export, Public Match API, Match Schema/data workflow, and actual Profile application without adding YouTube or EuroSkat integration before v1.0. |
