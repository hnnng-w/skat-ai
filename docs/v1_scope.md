...
Summary persistence/transport/UI, annotations, evaluation baselines, derived AI tags,
...
* Support usable manual post-game Match capture for one EuroSkat 36er Standard
  ...
  #178 subsequently add separate private immutable Learning Corpus Snapshot/
  ...
  and descriptive exact-Count cross-game summaries
  without changing
...
| --- | --- |
| Match capture | Issues #160 through #167 retain exact internal metadata, observed Games, 36-Slot Workspace/persistence, rapid-entry services, private no-JSON browser/autosave transport, Match-bound Statistics, information-safe Decision preparation, strict Historical and unpartitioned Training-source materialization, and complete fixed-list aggregation. Issue #168 adds explicit one-Decision Immediate/Search/Auto and strict Historical execution through one existing Application invocation, existing-behavior eligible Profile application, no-workflow materialization, deterministic max-eight ephemeral reports, concurrency invalidation, and authenticated canonical downloads. The functional local `v0.15.0` Package baseline is published at commit `ec1c154`. Issues #171 through #178 add private content-addressed Learning Corpus identity/persistence, derived Current-Snapshot Player/Statistics, Human and Strategy Teacher Evidence, Learning Dataset-v2, Match-group-safe partition preparation, and descriptive cross-game summaries. Deletion/garbage collection, Public Match API, Match Schema/data workflow, public/persisted Player Catalog, task-specific Dataset work, and database/remote deployment remain open; YouTube and EuroSkat integration are not required before v1.0. |
