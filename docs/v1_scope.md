...
| Externally captured opponent statistics | A versioned record of supplied total games and percentage-point statistics with stable player identity, source provenance, and capture time. | Normalization does not derive historical aggregates, confidence, policies, behavior predictions, or exact role-specific counts. |
...
| Historical opponent statistics | `decision required` | External captures now have a versioned contract, but historical-game aggregation, identity linking, time windows, privacy, and update behavior are not defined. |
