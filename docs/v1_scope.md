...
Match-bound Statistics history, and strict time-safe selection. Deletion and
garbage collection, Player Catalog persistence, persisted identity assertions,
merge/split operations, all-revision Player views, Commentary and Response export,
...
| Private Learning Corpus identity, Catalog, persistence, Workspace import, and derived Player history | Internal immutable content-addressed Match Snapshots built only from exact strictly resumed Workspace persistence documents, with exact closed References, lightweight entries, explicit Current selections, one fixed explicit local Store, strict Resume, valid orphan reporting, pure Catalog changes, no-clobber objects, optimistic atomic Catalog Save, source-preserving Workspace import, and one non-persisted Current-Snapshot Player Catalog with time-safe Statistics selection. | Issues #171 through #173 keep Match Workspaces as editable authority, imported Snapshot objects immutable, `catalog.json` authoritative, valid unreferenced objects as reported orphans, and Player history derived from explicit Current selections only. Future analysis/annotation artifacts and Dataset version `2` remain separate. No deletion, garbage collection, Player Catalog persistence, persisted aliases/assertions, merge/split, all-revision view, report import, Commentary export, browser, CLI, Public API, Schema, or model training exists. See [Learning Corpus identity and Catalogs](learning_corpus_identity_and_catalogs.md), [Learning Corpus persistence and Workspace import](learning_corpus_persistence_and_import.md), and [Learning Corpus Player Catalog and Statistics history](learning_corpus_player_catalog_and_statistics_history.md). |
