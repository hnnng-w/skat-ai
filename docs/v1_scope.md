...
after this focused milestone, and
...
| Local Match Capture interface | Internal version-1 Web, Protocol, and Capture CLI private transport with one explicit file, loopback token/same-origin server, no-JSON creation, 36-position UI, setup/Card/correction/annotation forms, CAS autosave, explicit Reload, and packaged progressive assets. | It is not a Root workflow or Public API and adds no remote bind, account, encryption claim, external source integration, analysis, materialization, Schema, example, or generated scenario. See [Local Match Capture interface](local_match_capture_interface.md). |
...
* Support usable manual post-game Match capture for one EuroSkat 36er Standard
  ...
  autosave transport. Materialization, Public Match API, Match Schema/data
  workflow, Player Statistics application, and analysis remain open.
...
| --- | --- |
| Match capture | Issues #160 through #164 retain exact internal metadata, observed-Game, 36-Slot Workspace/persistence, and transport-free rapid-entry contracts without hidden completion. Issue #165 adds the bounded private local no-JSON interface, exact/bounded Card presentation, correction/annotation forms, one-file CAS autosave, explicit Reload, loopback security, and installed/module/Legacy Capture CLI parity. Completion still requires materialization, Public Match API, Match Schema/data workflow, Player Statistics application, and analysis evidence without adding YouTube or EuroSkat integration before v1.0. |
