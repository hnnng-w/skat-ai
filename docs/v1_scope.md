...
local/CI distribution gates. Installed CLI entry points remain open. Issue #138 adds the internal
...
* Provide a stable library API and installed CLI/package interface. Public API
  ...
  metadata. Installed CLI entry points remain open.
...
| Regression testing | Ruff, packaged-schema parity, input example schema validation, generated-output schema validation, distribution build and clean-install validation, and the complete pytest suite all pass locally and in GitHub Actions from a clean checkout of the release candidate. |
