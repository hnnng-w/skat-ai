...
passed/clear wrappers. `v1.0.0` remains unready after this focused milestone, and
...
| Private Match Workspace | Internal immutable exact 36-position EuroSkat Workspace with empty, observed-Game, and passed-deal Slots; fixed rotation; revisions; Progress; fingerprints; strict Resume; and optimistic atomic local files. | Structural completeness differs from evidence completeness. Issue #164 wraps unchanged operations but performs no Load, Save, or autosave. Materialization, Public API, Match Schema, Match CLI, browser UI, distributed lock, retry/merge, cloud, encryption, and automatic backup remain absent. See [Match Workspace contracts](match_workspace_contracts.md). |
| Match Capture Application services | Internal transport-free rapid-entry operations over one loaded Workspace, with immutable Card entries, UI-ready Position Views, exact/bounded selectable Cards, automatic Player/Decision derivation, truncation cleanup, annotation editing, and revisioned Results. | Issue #164 implements the internal foundation without hidden completion, persistence orchestration, Public API, Match Schema/CLI, browser transport/UI, materialization, analysis, or tactical interpretation. See [Match Capture Application services](match_capture_application_services.md). |
...
* Support usable manual post-game Match capture for one EuroSkat 36er Standard
  ...
  wrappers. Materialization, persistence orchestration, Public API, Match Schema,
  CLI, browser transport, UI, and Player Statistics application remain open.
...
| --- | --- |
| Match capture | Internal version-1 metadata contracts retain one caller-supplied Match identity, descriptive YouTube/other-video/manual source, optional Match media bounds, exact `euroskat_36_standard_v1` format, three fixed-place participants, optional same-instant existing Opponent Statistics snapshots, and one perspective Player. Internal observed-Game contracts retain exact per-Game seats, optional Card facts, bounded partial or exact complete public Plays, free-text commentary, linked later responses, and deterministic evidence capabilities without hidden completion. Internal Workspace/persistence contracts add exactly 36 rotated empty/observed/passed Slots, revisions, Progress, fingerprints, strict Resume, and optimistic atomic private files. Internal Capture Application services add rapid-entry Position Views, exact/bounded Card palettes, setup updates, automatic append, truncation cleanup, annotation editing, and passed/clear wrappers without I/O or analysis. Completion still requires persistence/materialization orchestration, Public API, Match Schema, CLI, browser transport, UI, and Player Statistics application evidence without adding YouTube or EuroSkat integration before v1.0. |
