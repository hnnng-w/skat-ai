...
import, and persisted selection updates. Deletion and garbage collection, a
Player Catalog, multi-Match Player Snapshot history, Commentary and Response
export, separate behavior/strategy/communication targets, Dataset version `2`,
...
| Private Learning Corpus identity, Catalog, persistence, and Workspace import | Internal immutable content-addressed Match Snapshots built only from exact strictly resumed Workspace persistence documents, with exact closed References, lightweight entries, explicit Current selections, one fixed explicit local Store, strict Resume, valid orphan reporting, pure Catalog changes, no-clobber objects, optimistic atomic Catalog Save, and source-preserving Workspace import. | Issues #171 and #172 keep Match Workspaces as editable authority, imported Snapshot objects immutable, `catalog.json` authoritative, and valid unreferenced objects as reported orphans. Future analysis/annotation artifacts and Dataset version `2` remain separate. No deletion, garbage collection, Player Catalog, report import, Commentary export, browser, CLI, Public API, Schema, or model training exists. See [Learning Corpus identity and Catalogs](learning_corpus_identity_and_catalogs.md) and [Learning Corpus persistence and Workspace import](learning_corpus_persistence_and_import.md). |
...
* Support usable manual post-game Match capture for one EuroSkat 36er Standard
  ...
  local downloads, completing the functional local milestone. Issues #171 and
  #172 subsequently add separate private immutable Learning Corpus Snapshot/
  reference and lightweight Catalog identity, deterministic persistence, and
  Workspace-import foundations without changing Workspace authority. Public Match API,
