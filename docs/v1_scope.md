...
execution without adjudication or Runtime behavior. Every other current Claim
boundary remains `not_supported_v1`. Candidate directions include adjudication, Historical
...
* Represent complete historical games with structured claims, concessions, and
  ...
  preparation, and bounded proof execution but still requires adjudication, a
...
| Complete-game historical representation | A versioned schema and Runtime model represent stable game/player IDs, fixed seats, initial deal, final bid/declaration facts, Skat pickup/discards or Hand state, every play, structured Claims/concessions and approved additional end reasons, final result, and Settlement; valid records round-trip and inconsistent ownership, order, legality, totals, or outcomes are rejected. Matrix version `2` closes the Claim decision gate, and private party-wide Claim contracts, exact-state preparation, and bounded exhaustive five-Trick proof execution exist; adjudication, Historical integration, and Settlement reuse remain open. |
...
Private version-1 party-wide Claim contracts, one exact-state preparation, and
bounded exhaustive proof execution now exist outside these Runtime paths. This
does not close broader v1 gates for adjudication, Runtime/Historical integration,
public exposure, or complete Settlement coverage. Specific
