...
States, and current/ancestor/future/diverged lineage. Session-triggered analysis,
actual-card Checkpoint attachment, persistence, Public API,
Provenance propagation, Schemas, CLI, examples/generated output, automatic
Checkpoint collection, and UI remain open. Online-platform adapters, browser
extensions, and website scraping remain outside this bounded milestone.
...
| Interactive Session foundation | Immutable internal authoring State with exactly three stable seated Players, Live or Retrospective Mode, phase, accepted typed Command Log, linear revision, validation Diagnostics, Position/Historical readiness, deterministic full replay, frozen projection, atomic incremental Command application, immutable available/unavailable canonical Historical and Position Request export, and frozen local pre-Play Decision Checkpoints. | Issues #150 through #153 define and execute the internal transition, no-execution Request-export, declared-Ouvert public-hand, and Checkpoint boundaries; persistence, Undo/correction, Public API, Provenance, Schema, CLI, automatic Checkpoint collection, and UI remain absent. |
| Session History Edit and Checkpoint lineage | Immutable internal version-1 strict-prefix Undo, one-command replacement with deterministic original-suffix replay, valid partial corrected States, and exact Checkpoint relationships. | Issue #154 implements the internal boundary; the preceding row's Undo/correction absence now applies only to public or end-to-end exposure. It adds no persistence, automatic Redo stack, merging, arbitrary Log surgery, public API, Schema, CLI, or workflow execution. |
...
* Support interactive live and retrospective input and Session capture. Issue
  ...
  partial corrected States, and exact Checkpoint lineage. End-to-end capture,
  persistence, Public API, Provenance, Schemas, CLI, examples/generated output,
  automatic Checkpoint collection, and UI remain open.
