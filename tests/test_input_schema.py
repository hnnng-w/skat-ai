...
with HISTORICAL_DEFENDER_OPEN_PLAY_SCHEMA_PATH.open(
    "r", encoding="utf-8"
) as historical_defender_open_play_file:
...
with HISTORICAL_PARTY_WIDE_CLAIM_SCHEMA_PATH.open(
    "r", encoding="utf-8"
) as historical_party_wide_claim_file:
    HISTORICAL_PARTY_WIDE_CLAIM_SCHEMA = json.load(
        historical_party_wide_claim_file
    )
...
def test_schema_and_runtime_accept_historical_defender_open_play_branch() -> None:
    example_path = (
...
def test_training_dataset_schema_and_runtime_accept_historical_defender_open_play() -> None:
    data = build_valid_training_dataset_input()
    example_path = (
        PROJECT_ROOT / "examples" / "historical_grand_defender_open_play.json"
    )
    with example_path.open("r", encoding="utf-8") as example_file:
        open_play = json.load(example_file)["historical_game_input"]
    data["training_dataset_input"]["records"][0]["historical_game"] = open_play
